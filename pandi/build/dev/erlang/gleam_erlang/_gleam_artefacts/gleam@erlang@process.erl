...
-opaque subject(DQA) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DQA)}.

-type name(DQB) :: any() | {gleam_phantom, DQB}.

-type do_not_leak() :: any().

-type selector(DQC) :: any() | {gleam_phantom, DQC}.
...
-spec 'receive'(subject(DRA), integer()) -> {ok, DRA} | {error, nil}.
...
-spec receive_forever(subject(DRI)) -> DRI.
...
-spec selector_receive(selector(DRM), integer()) -> {ok, DRM} | {error, nil}.
...
-spec selector_receive_forever(selector(DRQ)) -> DRQ.
...
-spec map_selector(selector(DRS), fun((DRS) -> DRU)) -> selector(DRU).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRW), selector(DRW)) -> selector(DRW).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(DSA), fun((exit_message()) -> DSA)) -> selector(DSA).
...
-spec select_map(selector(DSH), subject(DSJ), fun((DSJ) -> DSH)) -> selector(DSH).
...
-spec select(selector(DSD), subject(DSD)) -> selector(DSD).
...
-spec deselect(selector(DSM), subject(any())) -> selector(DSM).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSR)
) -> selector(DSR).
...
-spec select_other(selector(DSV), fun((gleam@dynamic:dynamic_()) -> DSV)) -> selector(DSV).
...
-spec select_specific_monitor(selector(DTH), monitor(), fun((down()) -> DTH)) -> selector(DTH).
...
-spec select_monitors(selector(DTK), fun((down()) -> DTK)) -> selector(DTK).
...
-spec deselect_specific_monitor(selector(DTN), monitor()) -> selector(DTN).
...
-spec perform_call(
    subject(DTQ),
    fun((subject(DTS)) -> DTQ),
    fun((selector(DTS)) -> {ok, DTS} | {error, nil})
) -> DTS.
...
-spec call(subject(DTX), integer(), fun((subject(DTZ)) -> DTX)) -> DTZ.
...
-spec call_forever(subject(DUB), fun((subject(DUD)) -> DUB)) -> DUD.
...
-spec send_after(subject(DUJ), integer(), DUJ) -> timer().
