...
-opaque generator(QKX) :: {generator, fun((seed()) -> {QKX, seed()})}.
...
-spec step(generator(QKY), seed()) -> {QKY, seed()}.
step(Generator, Seed) ->
...
-spec constant(QLC) -> generator(QLC).
constant(Value) ->
    {generator, fun(Seed) -> {Value, Seed} end}.

-file("src/prng/random.gleam", 476).
-spec do_fixed_size_list(list(QMJ), seed(), generator(QMJ), integer()) -> {list(QMJ),
    seed()}.
...
-spec fixed_size_list(generator(QMF), integer()) -> generator(list(QMF)).
...
-spec then(generator(QPF), fun((QPF) -> generator(QPH))) -> generator(QPH).
...
-spec list(generator(QMN)) -> generator(list(QMN)).
list(Generator) ->
    then(int(0, 32), fun(_capture) -> fixed_size_list(Generator, _capture) end).

-file("src/prng/random.gleam", 387).
-spec get_by_weight({float(), QLW}, list({float(), QLW}), float()) -> QLW.
...
-spec map(generator(QPK), fun((QPK) -> QPM)) -> generator(QPM).
map(Generator, Fun) ->
    {generator,
        fun(Seed) ->
            {Value, Seed@1} = step(Generator, Seed),
...
-spec weighted({float(), QLM}, list({float(), QLM})) -> generator(QLM).
...
-spec uniform(QLE, list(QLE)) -> generator(QLE).
...
-spec try_uniform(list(QLH)) -> {ok, generator(QLH)} | {error, nil}.
try_uniform(Options) ->
    case Options of
        [First | Rest] ->
...
-spec try_weighted(list({float(), QLR})) -> {ok, generator(QLR)} | {error, nil}.
...
-spec choose(QLY, QLY) -> generator(QLY).
choose(One, Other) ->
    uniform(One, [Other]).
...
-spec pair(generator(QMA), generator(QMC)) -> generator({QMA, QMC}).
pair(One, Other) ->
...
-spec do_fixed_size_dict(
    ...
    integer(),
    integer(),
    gleam@dict:dict(QMW, QMY)
) -> generator(gleam@dict:dict(QMW, QMY)).
...
-spec fixed_size_dict(generator(QMR), generator(QMT), integer()) -> generator(gleam@dict:dict(QMR, QMT)).
...
-spec set(generator(QNT)) -> generator(gleam@set:set(QNT)).
set(Generator) ->
...
-spec fixed_size_set(generator(QNK), integer()) -> generator(gleam@set:set(QNK)).
...
-spec do_shuffle(list(QOC), list({float(), QOC})) -> generator(list(QOC)).
do_shuffle(List, Acc) ->
...
-spec shuffle(list(QNY)) -> generator(list(QNY)).
shuffle(List) ->
    do_shuffle(List, []).
...
-spec sample_loop(
    list(QOL),
    gleam@dict:dict(integer(), QOL),
    integer(),
    float()
) -> generator(gleam@dict:dict(integer(), QOL)).
...
-spec build_reservoir_loop(
    list(QOY),
    integer(),
    gleam@dict:dict(integer(), QOY)
) -> {gleam@dict:dict(integer(), QOY), list(QOY)}.
...
-spec build_reservoir(list(QOT), integer()) -> {gleam@dict:dict(integer(), QOT),
    list(QOT)}.
...
-spec sample(list(QOH), integer()) -> generator(list(QOH)).
sample(List, N) ->
...
-spec map2(generator(QPO), generator(QPQ), fun((QPO, QPQ) -> QPS)) -> generator(QPS).
map2(One, Other, Fun) ->
    {generator,
        fun(Seed) ->
