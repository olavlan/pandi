...
-type message(ECB) :: {message, ECB} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(ECC, ECD) :: {continue,
        ECC,
        gleam@option:option(gleam@erlang@process:selector(ECD))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(ECE, ECF) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        ECE,
        gleam@erlang@process:selector(message(ECF)),
        gleam@otp@system:debug_state(),
        fun((ECE, ECF) -> next(ECE, ECF))}.

-type started(ECG) :: {started, gleam@erlang@process:pid_(), ECG}.

-opaque initialised(ECH, ECI, ECJ) :: {initialised,
        ECH,
        gleam@option:option(gleam@erlang@process:selector(ECI)),
        ECJ}.

-opaque builder(ECK, ECL, ECM) :: {builder,
        fun((gleam@erlang@process:subject(ECL)) -> {ok,
                initialised(ECK, ECL, ECM)} |
            {error, binary()}),
        integer(),
        fun((ECK, ECL) -> next(ECK, ECL)),
        gleam@option:option(gleam@erlang@process:name(ECL))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(ECN) :: {ack, {ok, ECN} | {error, binary()}} |
...
-spec continue(ECS) -> next(ECS, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(EDE, EDF), gleam@erlang@process:selector(EDF)) -> next(EDE, EDF).
...
-spec initialised(EDL) -> initialised(EDL, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(EDQ, any(), EDS),
    gleam@erlang@process:selector(EDW)
) -> initialised(EDQ, EDW, EDS).
...
-spec returning(initialised(EEB, EEC, any()), EEH) -> initialised(EEB, EEC, EEH).
...
-spec new(EEL) -> builder(EEL, EEM, gleam@erlang@process:subject(EEM)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(EER)) -> {ok, initialised(EET, EER, EEU)} |
        {error, binary()})
) -> builder(EET, EER, EEU).
...
-spec on_message(builder(EFD, EFE, EFF), fun((EFD, EFE) -> next(EFD, EFE))) -> builder(EFD, EFE, EFF).
...
-spec named(builder(EFO, EFP, EFQ), gleam@erlang@process:name(EFP)) -> builder(EFO, EFP, EFQ).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(EGD))) -> gleam@erlang@process:selector(message(EGD)).
...
-spec receive_message(self(any(), EFZ)) -> message(EFZ).
...
-spec initialise_actor(
    builder(any(), any(), EGW),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EGW} | {error, binary()})
...
-spec start(builder(any(), any(), EHJ)) -> {ok, started(EHJ)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EHQ), EHQ) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EHS),
    integer(),
    fun((gleam@erlang@process:subject(EHU)) -> EHS)
) -> EHU.
