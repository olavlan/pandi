...
-opaque supervisor(EXU, EXV) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(EXU, EXV))}.

-type message(EXW, EXX) :: any() | {gleam_phantom, EXW, EXX}.

-opaque builder(EXY, EXZ) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((EXY) -> {ok, gleam@otp@actor:started(EXZ)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(EXY, EXZ)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(EYA, EYB) :: {local,
        gleam@erlang@process:name(message(EYA, EYB))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(EYC) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, EYC}.
...
-type erlang_child_spec_property(EYD, EYE) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((EYD) -> {ok, gleam@otp@actor:started(EYE)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(EYF, EYG))) -> supervisor(EYF, EYG).
get_by_name(Name) ->
...
-spec worker_child(
    fun((EYM) -> {ok, gleam@otp@actor:started(EYN)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EYM, EYN).
...
-spec supervisor_child(
    fun((EYR) -> {ok, gleam@otp@actor:started(EYS)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EYR, EYS).
...
-spec named(builder(EYW, EYX), gleam@erlang@process:name(message(EYW, EYX))) -> builder(EYW, EYX).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(EZF, EZG), integer(), integer()) -> builder(EZF, EZG).
...
-spec timeout(builder(EZL, EZM), integer()) -> builder(EZL, EZM).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(EZR, EZS), gleam@otp@supervision:restart()) -> builder(EZR, EZS).
...
-spec start(builder(EZX, EZY)) -> {ok,
        gleam@otp@actor:started(supervisor(EZX, EZY))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(FAW, FAX)) -> gleam@otp@supervision:child_specification(supervisor(FAW, FAX)).
supervised(Builder) ->
...
-spec start_child(supervisor(FBD, FBE), FBD) -> {ok,
        gleam@otp@actor:started(FBE)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((FCB) -> {ok, gleam@otp@actor:started(FCC)} |
        {error, gleam@otp@actor:start_error()}),
    FCB
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), FCC, gleam@otp@actor:start_error()).
