...
-type child_specification(EVG) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(EVG)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(EVH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EVH).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(EVM)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EVM).
supervisor(Start) ->
...
-spec significant(child_specification(EVR), boolean()) -> child_specification(EVR).
...
-spec timeout(child_specification(EVU), integer()) -> child_specification(EVU).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EVX), restart()) -> child_specification(EVX).
...
-spec map_data(child_specification(EWA), fun((EWA) -> EWC)) -> child_specification(EWC).
