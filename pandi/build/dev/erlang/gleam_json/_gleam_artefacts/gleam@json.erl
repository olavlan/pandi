...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DZT)) -> {ok, DZT} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DZN)) -> {ok, DZN} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DZJ)) -> {ok, DZJ} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DZZ), fun((DZZ) -> json())) -> json().
...
-spec array(list(EAD), fun((EAD) -> json())) -> json().
