...
-type tree(RIS) :: {tree, RIS, gleam@yielder:yielder(tree(RIS))}.

-file("src/qcheck/tree.gleam", 39).
-spec new(RIT, fun((RIT) -> gleam@yielder:yielder(RIT))) -> tree(RIT).
new(X, Shrink) ->
    Shrink_trees = begin
        _pipe = Shrink(X),
...
-spec map(tree(RIW), fun((RIW) -> RIY)) -> tree(RIY).
map(Tree, F) ->
    {tree, X, Xs} = Tree,
    Y = F(X),
...
-spec bind(tree(RJA), fun((RJA) -> tree(RJC))) -> tree(RJC).
bind(Tree, F) ->
    {tree, X, Xs} = Tree,
    {tree, Y, Ys_of_x} = F(X),
...
-spec apply(tree(fun((RJF) -> RJG)), tree(RJF)) -> tree(RJG).
apply(F, X) ->
    {tree, X0, Xs} = X,
    {tree, F0, Fs} = F,
    Y = F0(X0),
...
-spec return(RJK) -> tree(RJK).
return(X) ->
    {tree, X, gleam@yielder:empty()}.

-file("src/qcheck/tree.gleam", 192).
-spec parameter(fun((RKT) -> RKU)) -> fun((RKT) -> RKU).
parameter(F) ->
    F.

-file("src/qcheck/tree.gleam", 86).
-spec map2(tree(RJM), tree(RJO), fun((RJM, RJO) -> RJQ)) -> tree(RJQ).
map2(A, B, F) ->
    _pipe = begin
...
-spec yielder_cons(RJX, fun(() -> gleam@yielder:yielder(RJX))) -> gleam@yielder:yielder(RJX).
yielder_cons(Element, Yielder) ->
    gleam@yielder:yield(Element, Yielder).

-file("src/qcheck/tree.gleam", 112).
-spec option(tree(RKA)) -> tree(gleam@option:option(RKA)).
option(Tree) ->
    {tree, X, Xs} = Tree,
...
-spec do_collect(tree(RKI), fun((RKI) -> RKK), list(RKK)) -> list(RKK).
...
-spec collect(tree(RKE), fun((RKE) -> RKG)) -> list(RKG).
collect(Tree, F) ->
    do_collect(Tree, F, []).

-file("src/qcheck/tree.gleam", 159).
-spec do_to_string(tree(RKR), fun((RKR) -> binary()), integer(), integer()) -> binary().
...
-spec to_string(tree(RKN), fun((RKN) -> binary())) -> binary().
...
-spec to_string_max_depth(tree(RKP), fun((RKP) -> binary()), integer()) -> binary().
