...
-type generator(ROE) :: {generator,
        fun((qcheck@random:seed()) -> {qcheck@tree:tree(ROE),
            qcheck@random:seed()})}.
...
-type 'try'(ROF) :: {no_panic, ROF} | {panic, exception:exception()}.

-file("src/qcheck.gleam", 2799).
-spec failwith(RZH, RZH, integer(), exception:exception()) -> any().
...
-spec do_run_property(fun((ROP) -> nil), ROP, integer(), integer()) -> run_property_result().
...
-spec run_property(fun((ROO) -> nil), ROO, integer()) -> run_property_result().
run_property(Property, Value, Max_retries) ->
    do_run_property(Property, Value, Max_retries, 0).

-file("src/qcheck.gleam", 408).
-spec do_filter_map(
    gleam@yielder:yielder(ROZ),
    fun((ROZ) -> gleam@option:option(RPB))
) -> gleam@yielder:step(RPB, gleam@yielder:yielder(ROZ)).
do_filter_map(It, F) ->
...
-spec filter_map(
    gleam@yielder:yielder(ROU),
    fun((ROU) -> gleam@option:option(ROW))
) -> gleam@yielder:yielder(ROW).
filter_map(It, F) ->
    gleam@yielder:unfold(It, fun(_capture) -> do_filter_map(_capture, F) end).

-file("src/qcheck.gleam", 372).
-spec do_shrink(qcheck@tree:tree(ROS), fun((ROS) -> nil), integer(), integer()) -> {ROS,
    integer()}.
...
-spec shrink(qcheck@tree:tree(ROQ), fun((ROQ) -> nil), integer()) -> {ROQ,
    integer()}.
...
-spec do_run(config(), generator(ROM), fun((ROM) -> nil), integer()) -> nil.
...
-spec run(config(), generator(ROI), fun((ROI) -> nil)) -> nil.
...
-spec given(generator(ROK), fun((ROK) -> nil)) -> nil.
...
-spec generator(
    qcheck@random:generator(RPG),
    fun((RPG) -> qcheck@tree:tree(RPG))
) -> generator(RPG).
...
-spec generate(generator(RPK), integer(), qcheck@random:seed()) -> {list(RPK),
    qcheck@random:seed()}.
...
-spec generate_tree(generator(RPR), qcheck@random:seed()) -> {qcheck@tree:tree(RPR),
    qcheck@random:seed()}.
...
-spec constant(RPW) -> generator(RPW).
constant(A) ->
    return(A).
...
-spec parameter(fun((RPY) -> RPZ)) -> fun((RPY) -> RPZ).
parameter(F) ->
    F.
...
-spec map(generator(RQA), fun((RQA) -> RQC)) -> generator(RQC).
map(Generator, F) ->
    {generator, Generate} = Generator,
    {generator,
        fun(Seed) ->
...
-spec bind(generator(RQE), fun((RQE) -> generator(RQG))) -> generator(RQG).
bind(Generator, F) ->
    {generator, Generate} = Generator,
    {generator,
        fun(Seed) ->
...
-spec then(generator(RQJ), fun((RQJ) -> generator(RQL))) -> generator(RQL).
then(Generator, F) ->
    bind(Generator, F).
...
-spec apply(generator(fun((RQO) -> RQP)), generator(RQO)) -> generator(RQP).
apply(F, X) ->
    {generator, F@1} = F,
    {generator, X@1} = X,
    {generator,
...
-spec map2(generator(RQT), generator(RQV), fun((RQT, RQV) -> RQX)) -> generator(RQX).
map2(G1, G2, F) ->
    _pipe = return(
        begin
...
-spec map6(
    ...
    generator(RSL),
    generator(RSN),
    fun((RSD, RSF, RSH, RSJ, RSL, RSN) -> RSP)
) -> generator(RSP).
...
-spec tuple2(generator(RSR), generator(RST)) -> generator({RSR, RST}).
tuple2(G1, G2) ->
...
-spec tuple3(generator(RSW), generator(RSY), generator(RTA)) -> generator({RSW,
    RSY,
    RTA}).
tuple3(G1, G2, G3) ->
...
-spec tuple4(generator(RTD), generator(RTF), generator(RTH), generator(RTJ)) -> generator({RTD,
    RTF,
    RTH,
    RTJ}).
tuple4(G1, G2, G3, G4) ->
...
-spec tuple6(
    ...
    generator(RUF),
    generator(RUH)
) -> generator({RTX, RTZ, RUB, RUD, RUF, RUH}).
...
-spec from_generators(generator(RUK), list(generator(RUK))) -> generator(RUK).
...
-spec from_weighted_generators(
    {integer(), generator(RUP)},
    list({integer(), generator(RUP)})
) -> generator(RUP).
...
-spec sized_from(fun((integer()) -> generator(RUU)), generator(integer())) -> generator(RUU).
...
-spec list_cons(RZL, list(RZL)) -> list(RZL).
list_cons(X, Xs) ->
    [X | Xs].
...
-spec generic_list_loop(
    integer(),
    qcheck@tree:tree(list(RWL)),
    generator(RWL),
    qcheck@random:seed()
) -> {qcheck@tree:tree(list(RWL)), qcheck@random:seed()}.
...
-spec fixed_length_list_from(generator(RWW), integer()) -> generator(list(RWW)).
...
-spec generic_list(generator(RWR), generator(integer())) -> generator(list(RWR)).
...
-spec list_from(generator(RXA)) -> generator(list(RXA)).
...
-spec generic_dict(generator(RXE), generator(RXG), generator(integer())) -> generator(gleam@dict:dict(RXE, RXG)).
...
-spec generic_set(generator(RXM), generator(integer())) -> generator(gleam@set:set(RXM)).
...
-spec option_from(generator(RXS)) -> generator(gleam@option:option(RXS)).
