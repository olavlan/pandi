...
-type action(FNT) :: stop | {continue, FNT, fun(() -> action(FNT))}.

-opaque yielder(FNU) :: {yielder, fun(() -> action(FNU))}.

-type step(FNV, FNW) :: {next, FNV, FNW} | done.

-type chunk(FNX, FNY) :: {another_by,
        list(FNX),
        FNY,
        FNX,
        fun(() -> action(FNX))} |
...
-type sized_chunk(FNZ) :: {another, list(FNZ), fun(() -> action(FNZ))} |
    {last, list(FNZ)} |
    no_more.
...
-spec unfold_loop(FOH, fun((FOH) -> step(FOI, FOH))) -> fun(() -> action(FOI)).
...
-spec unfold(FOC, fun((FOC) -> step(FOD, FOC))) -> yielder(FOD).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> FOM)) -> yielder(FOM).
repeatedly(F) ->
...
-spec from_list(list(FOQ)) -> yielder(FOQ).
from_list(List) ->
...
-spec transform(yielder(FPA), FPC, fun((FPC, FPA) -> step(FPD, FPC))) -> yielder(FPD).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(FPK)), fun((FPM, FPK) -> FPM), FPM) -> FPM.
...
-spec fold(yielder(FPH), FPJ, fun((FPJ, FPH) -> FPJ)) -> FPJ.
...
-spec to_list(yielder(FPP)) -> list(FPP).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(FPS)) -> step(FPS, yielder(FPS)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(FQA)), integer()) -> fun(() -> action(FQA)).
...
-spec take(yielder(FPX), integer()) -> yielder(FPX).
...
-spec drop_loop(fun(() -> action(FQG)), integer()) -> action(FQG).
...
-spec map(yielder(FQJ), fun((FQJ) -> FQL)) -> yielder(FQL).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(FQR), yielder(FQT), fun((FQR, FQT) -> FQV)) -> yielder(FQV).
...
-spec append_loop(fun(() -> action(FRH)), fun(() -> action(FRH))) -> action(FRH).
...
-spec append(yielder(FRD), yielder(FRD)) -> yielder(FRD).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(FRP)))) -> action(FRP).
...
-spec flatten(yielder(yielder(FRL))) -> yielder(FRL).
flatten(Yielder) ->
...
-spec concat(list(yielder(FRT))) -> yielder(FRT).
concat(Yielders) ->
...
-spec flat_map(yielder(FRX), fun((FRX) -> yielder(FRZ))) -> yielder(FRZ).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(FSF)), fun((FSF) -> boolean())) -> action(FSF).
...
-spec filter(yielder(FSC), fun((FSC) -> boolean())) -> yielder(FSC).
...
-spec filter_map(yielder(FSI), fun((FSI) -> {ok, FSK} | {error, any()})) -> yielder(FSK).
...
-spec cycle(yielder(FSW)) -> yielder(FSW).
cycle(Yielder) ->
...
-spec find_loop(fun(() -> action(FTE)), fun((FTE) -> boolean())) -> {ok, FTE} |
    {error, nil}.
...
-spec find(yielder(FTA), fun((FTA) -> boolean())) -> {ok, FTA} | {error, nil}.
...
-spec find_map(yielder(FTI), fun((FTI) -> {ok, FTK} | {error, any()})) -> {ok,
        FTK} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(FUB)), integer()) -> fun(() -> action({FUB,
    integer()})).
...
-spec index(yielder(FTY)) -> yielder({FTY, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(FUE, fun((FUE) -> FUE)) -> yielder(FUE).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(FUJ)), fun((FUJ) -> boolean())) -> fun(() -> action(FUJ)).
...
-spec take_while(yielder(FUG), fun((FUG) -> boolean())) -> yielder(FUG).
...
-spec drop_while_loop(fun(() -> action(FUP)), fun((FUP) -> boolean())) -> action(FUP).
...
-spec drop_while(yielder(FUM), fun((FUM) -> boolean())) -> yielder(FUM).
...
-spec scan_loop(fun(() -> action(FUW)), fun((FUY, FUW) -> FUY), FUY) -> fun(() -> action(FUY)).
...
-spec scan(yielder(FUS), FUU, fun((FUU, FUS) -> FUU)) -> yielder(FUU).
...
-spec zip_loop(fun(() -> action(FVF)), fun(() -> action(FVH))) -> fun(() -> action({FVF,
    FVH})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(FVA), yielder(FVC)) -> yielder({FVA, FVC}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(FVU)), fun((FVU) -> FVW), FVW, list(FVU)) -> chunk(FVU, FVW).
...
-spec chunk_loop(fun(() -> action(FVP)), fun((FVP) -> FVR), FVR, FVP) -> action(list(FVP)).
...
-spec chunk(yielder(FVK), fun((FVK) -> any())) -> yielder(list(FVK)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(FWI)), integer(), list(FWI)) -> sized_chunk(FWI).
...
-spec sized_chunk_loop(fun(() -> action(FWE)), integer()) -> fun(() -> action(list(FWE))).
...
-spec sized_chunk(yielder(FWA), integer()) -> yielder(list(FWA)).
...
-spec intersperse_loop(fun(() -> action(FWP)), FWP) -> action(FWP).
...
-spec intersperse(yielder(FWM), FWM) -> yielder(FWM).
...
-spec any_loop(fun(() -> action(FWU)), fun((FWU) -> boolean())) -> boolean().
...
-spec any(yielder(FWS), fun((FWS) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(FWY)), fun((FWY) -> boolean())) -> boolean().
...
-spec all(yielder(FWW), fun((FWW) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(FXO) -> fun((gleam@option:option(list(FXO))) -> list(FXO)).
...
-spec group_updater(fun((FXG) -> FXH)) -> fun((gleam@dict:dict(FXH, list(FXG)), FXG) -> gleam@dict:dict(FXH, list(FXG))).
group_updater(F) ->
...
-spec group(yielder(FXA), fun((FXA) -> FXC)) -> gleam@dict:dict(FXC, list(FXA)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(FXS), fun((FXS, FXS) -> FXS)) -> {ok, FXS} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(FXW)) -> {ok, FXW} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(FYE) -> yielder(FYE).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(FYK)), fun(() -> action(FYK))) -> action(FYK).
...
-spec interleave(yielder(FYG), yielder(FYG)) -> yielder(FYG).
...
-spec fold_until_loop(
    fun(() -> action(FYS)),
    fun((FYU, FYS) -> gleam@list:continue_or_stop(FYU)),
    FYU
) -> FYU.
...
-spec fold_until(
    yielder(FYO),
    FYQ,
    fun((FYQ, FYO) -> gleam@list:continue_or_stop(FYQ))
) -> FYQ.
...
-spec try_fold(yielder(FYW), FYY, fun((FYY, FYW) -> {ok, FYY} | {error, FYZ})) -> {ok,
        FYY} |
    {error, FYZ}.
...
-spec first(yielder(FZM)) -> {ok, FZM} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(FZQ), integer()) -> {ok, FZQ} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(FZY), fun((FZY) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(GAB, fun(() -> yielder(GAB))) -> yielder(GAB).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(GAE), GAE) -> yielder(GAE).
