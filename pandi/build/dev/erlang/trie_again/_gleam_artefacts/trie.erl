...
-export([new/0, delete/2, fold/3, insert/3, from_list/1, get/2, has_path/2, size/1, is_empty/1, map/2, paths/1, singleton/2, subtrie/2, to_list/1, update/3, values/1]).
-export_type([trie/2]).
...
-opaque trie(FQM, FQN) :: {trie,
        gleam@option:option(FQN),
        gleam@dict:dict(FQM, trie(FQM, FQN))}.
...
-spec do_delete(trie(FQV, FQW), list(FQV)) -> gleam@option:option(trie(FQV, FQW)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec delete(trie(FQO, FQP), list(FQO)) -> trie(FQO, FQP).
delete(Trie, Path) ->
...
-spec fold(trie(FRD, FRE), FRH, fun((FRH, list(FRD), FRE) -> FRH)) -> FRH.
...
-spec insert(trie(FSB, FSC), list(FSB), FSC) -> trie(FSB, FSC).
...
-spec from_list(list({list(FRJ), FRL})) -> trie(FRJ, FRL).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec get(trie(FRP, FRQ), list(FRP)) -> {ok, FRQ} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(FRW, any()), list(FRW)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(FSM, FSN), fun((FSN) -> FSQ)) -> trie(FSM, FSQ).
map(Trie, Fun) ->
    {trie,
...
-spec paths(trie(FSX, any())) -> list(list(FSX)).
paths(Trie) ->
...
-spec singleton(list(FTD), FTF) -> trie(FTD, FTF).
...
-spec subtrie(trie(FTM, FTN), list(FTM)) -> {ok, trie(FTM, FTN)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
...
-spec to_list(trie(FTV, FTW)) -> list({list(FTV), FTW}).
to_list(Trie) ->
...
-spec do_update(
    trie(FUK, FUL),
    list(FUK),
    fun((gleam@option:option(FUL)) -> gleam@option:option(FUL))
) -> gleam@option:option(trie(FUK, FUL)).
...
-spec update(
    trie(FUB, FUC),
    list(FUB),
    fun((gleam@option:option(FUC)) -> gleam@option:option(FUC))
) -> trie(FUB, FUC).
...
-spec values(trie(any(), FUV)) -> list(FUV).
values(Trie) ->
