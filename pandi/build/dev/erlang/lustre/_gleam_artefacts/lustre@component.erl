...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, NCZ} | {error, nil})) -> lustre@runtime@app:option(NCZ).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(NDD)) -> lustre@runtime@app:option(NDD).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(NDG)) -> lustre@runtime@app:option(NDG).
...
-spec on_form_autofill(fun((binary()) -> NDL)) -> lustre@runtime@app:option(NDL).
...
-spec on_form_reset(NDN) -> lustre@runtime@app:option(NDN).
...
-spec on_form_restore(fun((binary()) -> NDP)) -> lustre@runtime@app:option(NDP).
...
-spec on_connect(NDX) -> lustre@runtime@app:option(NDX).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(NDZ) -> lustre@runtime@app:option(NDZ).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(NEB) -> lustre@runtime@app:option(NEB).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(NED)),
    list(lustre@vdom@vnode:element(NED))
) -> lustre@vdom@vnode:element(NED).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(NEJ)),
    list(lustre@vdom@vnode:element(NEJ))
) -> lustre@vdom@vnode:element(NEJ).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), NFJ),
    binary(),
    list(lustre@vdom@vattr:attribute(NFJ)),
    list(lustre@vdom@vnode:element(NFJ))
) -> lustre@vdom@vnode:element(NFJ).
