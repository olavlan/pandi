...
-export([none/0, from/1, before_paint/1, after_paint/1, event/2, select/1, provide/2, batch/1, map/2, perform/6]).
...
-opaque effect(IAN) :: {effect,
        list(fun((actions(IAN)) -> nil)),
        list(fun((actions(IAN)) -> nil)),
        list(fun((actions(IAN)) -> nil))}.

-type actions(IAO) :: {actions,
        fun((IAO) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(IAO)) -> nil),
...
-spec from(fun((fun((IAR) -> nil)) -> nil)) -> effect(IAR).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((IAT) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(IAT).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((IAV) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(IAV).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((IAZ) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(IAZ))
) -> effect(IAZ).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(IBG))) -> effect(IBG).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
...
-spec do_comap_select(
    actions(IBY),
    gleam@erlang@process:selector(ICA),
    fun((ICA) -> IBY)
) -> nil.
...
-spec do_comap_actions(actions(IBU), fun((IBW) -> IBU)) -> actions(IBW).
...
-spec do_map(list(fun((actions(IBO)) -> nil)), fun((IBO) -> IBR)) -> list(fun((actions(IBR)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(IBK), fun((IBK) -> IBM)) -> effect(IBM).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((ICC) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(ICC)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
