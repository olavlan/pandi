...
-spec on(binary(), gleam@dynamic@decode:decoder(PYY)) -> lustre@vdom@vattr:attribute(PYY).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PZB))
) -> lustre@vdom@vattr:attribute(PZB).
...
-spec handler(PZF, boolean(), boolean()) -> lustre@vdom@vattr:handler(PZF).
...
-spec prevent_default(lustre@vdom@vattr:attribute(PZH)) -> lustre@vdom@vattr:attribute(PZH).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(PZK)) -> lustre@vdom@vattr:attribute(PZK).
...
-spec debounce(lustre@vdom@vattr:attribute(PZN), integer()) -> lustre@vdom@vattr:attribute(PZN).
...
-spec throttle(lustre@vdom@vattr:attribute(PZQ), integer()) -> lustre@vdom@vattr:attribute(PZQ).
...
-spec on_keypress(fun((binary()) -> QAH)) -> lustre@vdom@vattr:attribute(QAH).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> QAJ)) -> lustre@vdom@vattr:attribute(QAJ).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> QAL)) -> lustre@vdom@vattr:attribute(QAL).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> QAN)) -> lustre@vdom@vattr:attribute(QAN).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> QAP)) -> lustre@vdom@vattr:attribute(QAP).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> QAR)) -> lustre@vdom@vattr:attribute(QAR).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> QAU)) -> lustre@vdom@vattr:attribute(QAU).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(QAY) -> lustre@vdom@vattr:attribute(QAY).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(QBA) -> lustre@vdom@vattr:attribute(QBA).
on_blur(Msg) ->
