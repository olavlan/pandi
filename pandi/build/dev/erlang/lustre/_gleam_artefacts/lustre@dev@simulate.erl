...
-opaque app(OOY, OOZ, OPA) :: {app,
        fun((OOY) -> {OOZ, lustre@effect:effect(OPA)}),
        fun((OOZ, OPA) -> {OOZ, lustre@effect:effect(OPA)}),
        fun((OOZ) -> lustre@vdom@vnode:element(OPA))}.

-opaque simulation(OPB, OPC) :: {simulation,
        fun((OPB, OPC) -> {OPB, lustre@effect:effect(OPC)}),
        fun((OPB) -> lustre@vdom@vnode:element(OPC)),
        list(event(OPC)),
        OPB,
        lustre@vdom@vnode:element(OPC)}.

-type event(OPD) :: {dispatch, OPD} |
...
-spec simple(
    fun((OPE) -> OPF),
    fun((OPF, OPG) -> OPF),
    fun((OPF) -> lustre@vdom@vnode:element(OPG))
) -> app(OPE, OPF, OPG).
...
-spec application(
    fun((OPL) -> {OPM, lustre@effect:effect(OPN)}),
    fun((OPM, OPN) -> {OPM, lustre@effect:effect(OPN)}),
    fun((OPM) -> lustre@vdom@vnode:element(OPN))
) -> app(OPL, OPM, OPN).
...
-spec start(app(OPU, OPV, OPW), OPU) -> simulation(OPV, OPW).
start(App, Args) ->
...
-spec message(simulation(OQC, OQD), OQD) -> simulation(OQC, OQD).
...
-spec problem(simulation(ORI, ORJ), binary(), binary()) -> simulation(ORI, ORJ).
...
-spec click(simulation(OQP, OQQ), lustre@dev@query:'query'()) -> simulation(OQP, OQQ).
...
-spec input(simulation(OQV, OQW), lustre@dev@query:'query'(), binary()) -> simulation(OQV, OQW).
...
-spec model(simulation(ORO, any())) -> ORO.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), ORT)) -> lustre@vdom@vnode:element(ORT).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), ORY)) -> list(event(ORY)).
history(Simulation) ->
