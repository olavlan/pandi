...
-type state(LRR, LRS) :: {state,
        gleam@erlang@process:subject(message(LRS)),
        gleam@erlang@process:selector(message(LRS)),
        gleam@erlang@process:selector(message(LRS)),
        LRR,
        fun((LRR, LRS) -> {LRR, lustre@effect:effect(LRS)}),
        fun((LRR) -> lustre@vdom@vnode:element(LRS)),
        config(LRS),
        lustre@vdom@vnode:element(LRS),
        lustre@vdom@cache:cache(LRS),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(LRS)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(LRS)) -> nil))}.

-type config(LRT) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, LRT} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(LRT)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(LRT)),
        gleam@option:option(LRT),
        gleam@option:option(LRT)}.

-type message(LRU) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(LRU))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(LRU))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(LRU)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(LRU)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(LRU))} |
    {effect_dispatched_message, LRU} |
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(LTU)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(LTU)) -> nil)),
    lustre@runtime@transport:client_message(LTU)
) -> nil.
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(LTQ)),
    lustre@effect:effect(LTQ)
) -> nil.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(LTK)),
    binary(),
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, LTD} | {error, nil})),
    binary(),
    binary()
) -> {ok, LTD} | {error, nil}.
...
?DOC(false).
-spec loop(state(LSN, LSO), message(LSO)) -> gleam@otp@actor:next(state(LSN, LSO), message(LSO)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(LRY))),
    fun((LSC) -> {LSD, lustre@effect:effect(LRY)}),
    fun((LSD, LRY) -> {LSD, lustre@effect:effect(LRY)}),
    fun((LSD) -> lustre@vdom@vnode:element(LRY)),
    config(LRY),
    LSC
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(LRY)))} |
