...
-type element(IKE) :: {fragment,
        integer(),
        binary(),
        list(element(IKE)),
        lustre@internals@mutable_map:mutable_map(binary(), element(IKE))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(IKE)),
        list(element(IKE)),
        lustre@internals@mutable_map:mutable_map(binary(), element(IKE)),
        boolean(),
        boolean()} |
...
-spec fragment(
    binary(),
    list(element(IKM)),
    lustre@internals@mutable_map:mutable_map(binary(), element(IKM))
) -> element(IKM).
...
-spec element(
    ...
    list(element(IKT)),
    lustre@internals@mutable_map:mutable_map(binary(), element(IKT)),
    boolean(),
    boolean()
) -> element(IKT).
element(
...
?DOC(false).
-spec map(element(ILI), fun((ILI) -> ILK)) -> element(ILK).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(ILN))) -> element(ILN).
memo(Key, Dependencies, View) ->
    {memo, 5, Key, Dependencies, View}.

-file("src/lustre/vdom/vnode.gleam", 201).
?DOC(false).
-spec to_keyed(binary(), element(ILS)) -> element(ILS).
to_keyed(Key, Node) ->
    case Node of
...
-spec memo_to_json(
    fun(() -> element(ILV)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(ILV)), element(ILV))
) -> gleam@json:json().
...
-spec element_to_json(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(ILV)),
    list(element(ILV)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(ILV)), element(ILV))
) -> gleam@json:json().
