...
-type runtime(NQW) :: any() | {gleam_phantom, NQW}.
...
-spec application(
    fun((NRS) -> {NRT, lustre@effect:effect(NRU)}),
    fun((NRT, NRU) -> {NRT, lustre@effect:effect(NRU)}),
    fun((NRT) -> lustre@vdom@vnode:element(NRU))
) -> lustre@runtime@app:app(NRS, NRT, NRU).
...
-spec element(lustre@vdom@vnode:element(NRF)) -> lustre@runtime@app:app(any(), nil, NRF).
element(View) ->
    application(
...
-spec simple(
    fun((NRL) -> NRM),
    fun((NRM, NRN) -> NRM),
    fun((NRM) -> lustre@vdom@vnode:element(NRN))
) -> lustre@runtime@app:app(NRL, NRM, NRN).
...
-spec component(
    fun((NSB) -> {NSC, lustre@effect:effect(NSD)}),
    fun((NSC, NSD) -> {NSC, lustre@effect:effect(NSD)}),
    fun((NSC) -> lustre@vdom@vnode:element(NSD)),
    list(lustre@runtime@app:option(NSD))
) -> lustre@runtime@app:app(NSB, NSC, NSD).
...
-spec named(
    lustre@runtime@app:app(NSM, NSN, NSO),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(NSO))
) -> lustre@runtime@app:app(NSM, NSN, NSO).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(NTG, any(), NTI), binary(), NTG) -> {ok,
        runtime(NTI)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec start(lustre@runtime@app:app(NSX, any(), NSZ), binary(), NSX) -> {ok,
        runtime(NSZ)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(NTP, any(), NTR), NTP) -> {ok,
        runtime(NTR)} |
    {error, error()}.
...
-spec supervised(lustre@runtime@app:app(NTY, any(), NUA), NTY) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(NUA))).
...
-spec factory(lustre@runtime@app:app(NUH, any(), NUJ)) -> gleam@otp@factory_supervisor:builder(NUH, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(NUJ))).
factory(App) ->
...
-spec send(runtime(NUY), lustre@runtime@server@runtime:message(NUY)) -> nil.
...
-spec dispatch(NVB) -> lustre@runtime@server@runtime:message(NVB).
dispatch(Message) ->
