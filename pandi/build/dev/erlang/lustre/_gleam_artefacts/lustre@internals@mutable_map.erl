...
-type mutable_map(IGW, IGX) :: any() | {gleam_phantom, IGW, IGX}.
...
?DOC(false).
-spec unsafe_get(mutable_map(IHC, IHD), IHC) -> IHD.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(IHK, IHL), IHK, fun(() -> IHL)) -> IHL.
...
?DOC(false).
-spec has_key(mutable_map(IHU, any()), IHU) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(IIC, IID), IIC, IID) -> mutable_map(IIC, IID).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(IIO, IIP), IIO) -> mutable_map(IIO, IIP).
delete(Map, Key) ->
    maps:remove(Key, Map).
