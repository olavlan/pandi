...
-type attribute(HBL) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(HBL)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(HBM) :: {handler, boolean(), boolean(), HBM}.
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(HBR)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
...
?DOC(false).
-spec merge(list(attribute(HCB)), list(attribute(HCB))) -> list(attribute(HCB)).
...
?DOC(false).
-spec compare(attribute(HCI), attribute(HCI)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(HBW))) -> list(attribute(HBW)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
