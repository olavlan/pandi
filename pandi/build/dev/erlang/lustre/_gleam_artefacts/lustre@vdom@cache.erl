...
-opaque cache(KPB) :: {cache,
        events(KPB),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KPB)), lustre@vdom@vnode:element(KPB)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KPB)), lustre@vdom@vnode:element(KPB)),
        list(binary()),
        list(binary())}.

-opaque events(KPC) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KPC))),
        lustre@internals@mutable_map:mutable_map(binary(), child(KPC))}.

-type child(KPD) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(KPD)}.

-type added_children(KPE) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KPE))),
        lustre@internals@mutable_map:mutable_map(binary(), child(KPE)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KPE)), lustre@vdom@vnode:element(KPE))}.

-opaque decoded_event(KPF) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(KPF)} |
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KRB))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KRB))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KRB))).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KSE))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(KSE))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KSE))).
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KSW))),
    lustre@internals@mutable_map:mutable_map(binary(), child(KSW)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KSW)), lustre@vdom@vnode:element(KSW)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(KSW))
) -> added_children(KSW).
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(KSP))
) -> {cache(KSP), events(KSP)}.
...
-spec add_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(KRY)
) -> {cache(KRY), events(KRY)}.
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(KPK)) -> cache(KPK).
from_node(Root) ->
    Cache = new(),
...
?DOC(false).
-spec events(cache(KPQ)) -> events(KPQ).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(KPT), events(KPT)) -> cache(KPT).
...
?DOC(false).
-spec memos(cache(KPX)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KPX)), lustre@vdom@vnode:element(KPX)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(KQA),
    fun(() -> lustre@vdom@vnode:element(KQA)),
    fun(() -> lustre@vdom@vnode:element(KQA))
) -> lustre@vdom@vnode:element(KQA).
...
-spec keep_memo(
    cache(KQF),
    fun(() -> lustre@vdom@vnode:element(KQF)),
    fun(() -> lustre@vdom@vnode:element(KQF))
) -> cache(KQF).
...
-spec add_memo(
    cache(KQK),
    fun(() -> lustre@vdom@vnode:element(KQK)),
    lustre@vdom@vnode:element(KQK)
) -> cache(KQK).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KQW))
) -> events(KQW).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KRP))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KRP))).
...
?DOC(false).
-spec remove_event(events(KRM), lustre@vdom@path:path(), binary()) -> events(KRM).
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KTN))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(KTN))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KTN))).
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KTY))),
    lustre@internals@mutable_map:mutable_map(binary(), child(KTY)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KTY)), lustre@vdom@vnode:element(KTY)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(KTY))
) -> events(KTY).
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(KUK),
    lustre@vdom@vnode:element(KUK)
) -> {cache(KUK), events(KUK)}.
...
?DOC(false).
-spec dispatch(cache(KUY), decoded_event(KUY)) -> {cache(KUY),
    {ok, lustre@vdom@vattr:handler(KUY)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(KVC), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(KVC),
    {ok, lustre@vdom@vattr:handler(KVC)} | {error, nil}}.
