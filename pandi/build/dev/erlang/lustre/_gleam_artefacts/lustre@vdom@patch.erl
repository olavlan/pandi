...
-export([new/4, replace_text/1, replace_inner_html/1, update/2, move/2, remove/1, replace/2, insert/2, is_empty/1, add_child/2, to_json/2]).
...
-type patch(KEA) :: {patch,
        integer(),
        integer(),
        list(change(KEA)),
        list(patch(KEA))}.

-type change(KEB) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(KEB)),
        list(lustre@vdom@vattr:attribute(KEB))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(KEB)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(KEB)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(KEC)), list(patch(KEC))) -> patch(KEC).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(KEM)),
    list(lustre@vdom@vattr:attribute(KEM))
) -> change(KEM).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(KEW)) -> change(KEW).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(KEZ)), integer()) -> change(KEZ).
...
?DOC(false).
-spec add_child(patch(KFF), patch(KFF)) -> patch(KFF).
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(KHW)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KHW)), lustre@vdom@vnode:element(KHW))
) -> gleam@json:json().
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(KHY),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KHY)), lustre@vdom@vnode:element(KHY))
) -> gleam@json:json().
...
-spec change_to_json(
    change(KFM),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KFM)), lustre@vdom@vnode:element(KFM))
) -> gleam@json:json().
...
-spec to_json(
    patch(KFJ),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KFJ)), lustre@vdom@vnode:element(KFJ))
) -> gleam@json:json().
to_json(Patch, Memos) ->
