...
-export([mount/8, reconcile/2, emit/2, provide/2, attribute_changed/2, event_fired/3, property_changed/2, batch/1, context_provided/2, client_message_to_json/1, server_message_decoder/0, context_provided_decoder/0]).
...
-type client_message(KJG) :: {mount,
        ...
        lustre@vdom@vnode:element(KJG),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KJG)), lustre@vdom@vnode:element(KJG))} |
    {reconcile,
        integer(),
        lustre@vdom@patch:patch(KJG),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KJG)), lustre@vdom@vnode:element(KJG))} |
...
-spec mount(
    ...
    list(binary()),
    gleam@dict:dict(binary(), gleam@json:json()),
    lustre@vdom@vnode:element(KJM),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KJM)), lustre@vdom@vnode:element(KJM))
) -> client_message(KJM).
mount(
    Open_shadow_root,
    Will_adopt_styles,
...
?DOC(false).
-spec reconcile(
    lustre@vdom@patch:patch(KJQ),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KJQ)), lustre@vdom@vnode:element(KJQ))
) -> client_message(KJQ).
...
-spec reconcile_to_json(
    integer(),
    lustre@vdom@patch:patch(KKJ),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KKJ)), lustre@vdom@vnode:element(KKJ))
) -> gleam@json:json().
...
-spec mount_to_json(
    ...
    list(binary()),
    gleam@dict:dict(binary(), gleam@json:json()),
    lustre@vdom@vnode:element(KKG),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(KKG)), lustre@vdom@vnode:element(KKG))
) -> gleam@json:json().
mount_to_json(
    Kind,
    Open_shadow_root,
