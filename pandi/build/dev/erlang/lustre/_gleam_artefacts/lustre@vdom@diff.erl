...
-type diff(LFV) :: {diff,
        lustre@vdom@patch:patch(LFV),
        lustre@vdom@cache:cache(LFV)}.

-type partial_diff(LFW) :: {partial_diff,
        lustre@vdom@patch:patch(LFW),
        lustre@vdom@cache:cache(LFW),
        lustre@vdom@cache:events(LFW)}.

-type attribute_change(LFX) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(LFX)),
        list(lustre@vdom@vattr:attribute(LFX)),
        lustre@vdom@cache:events(LFX)}.
...
-spec diff_attributes(
    ...
    lustre@vdom@cache:events(LHA),
    list(lustre@vdom@vattr:attribute(LHA)),
    list(lustre@vdom@vattr:attribute(LHA)),
    list(lustre@vdom@vattr:attribute(LHA)),
    list(lustre@vdom@vattr:attribute(LHA))
) -> attribute_change(LHA).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(LGD)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(LGD)),
    list(lustre@vdom@vnode:element(LGD)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(LGD)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(LGD)),
    list(lustre@vdom@patch:patch(LGD)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(LGD),
    lustre@vdom@cache:events(LGD)
) -> partial_diff(LGD).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(LFY),
    lustre@vdom@vnode:element(LFY),
    lustre@vdom@vnode:element(LFY)
) -> diff(LFY).
diff(Cache, Old, New) ->
