...
-spec push_key({ok, DTD} | {error, get_error()}, binary()) -> {ok, DTD} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec do(
    {ok, {DVT, list(binary())}} | {error, parse_error()},
    fun((DVT, list(binary())) -> {ok, DVW} | {error, parse_error()})
) -> {ok, DVW} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {DWB, list(binary())}} | {error, parse_error()})
) -> {ok, {DWB, list(binary())}} | {error, parse_error()}.
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {DVI, list(binary())}} | {error, parse_error()})
) -> {ok, {DVI, list(binary())}} | {error, parse_error()}.
