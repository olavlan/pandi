...
-spec be_ok({ok, GWG} | {error, any()}) -> GWG.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, GWL}) -> GWL.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(GWO)) -> GWO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
