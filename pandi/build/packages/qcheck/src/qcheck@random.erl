...
-export([seed/1, step/2, int/2, random_seed/0, float/2, float_weighted/2, weighted/2, uniform/2, choose/2, bind/2, then/2, map/2, to_yielder/2, to_random_yielder/1, sample/2, random_sample/1, constant/1]).
...
-opaque generator(LVK) :: {generator, prng@random:generator(LVK)}.
...
-spec step(generator(LVL), seed()) -> {LVL, seed()}.
step(Generator, Seed) ->
...
-spec float_weighted({float(), LVP}, list({float(), LVP})) -> generator(LVP).
float_weighted(First, Others) ->
    _pipe = prng@random:weighted(First, Others),
    {generator, _pipe}.

-file("src/qcheck/random.gleam", 112).
-spec get_by_weight({integer(), LWA}, list({integer(), LWA}), integer()) -> LWA.
...
-spec weighted({integer(), LVS}, list({integer(), LVS})) -> generator(LVS).
...
-spec uniform(LVV, list(LVV)) -> generator(LVV).
...
-spec choose(LVY, LVY) -> generator(LVY).
choose(One, Other) ->
    uniform(One, [Other]).

-file("src/qcheck/random.gleam", 126).
-spec bind(generator(LWC), fun((LWC) -> generator(LWE))) -> generator(LWE).
bind(Generator, F) ->
...
-spec then(generator(LWH), fun((LWH) -> generator(LWJ))) -> generator(LWJ).
then(Generator, F) ->
    bind(Generator, F).

-file("src/qcheck/random.gleam", 142).
-spec map(generator(LWM), fun((LWM) -> LWO)) -> generator(LWO).
map(Generator, Fun) ->
    _pipe = prng@random:map(erlang:element(2, Generator), Fun),
    {generator, _pipe}.

-file("src/qcheck/random.gleam", 150).
-spec to_yielder(generator(LWT), seed()) -> gleam@yielder:yielder(LWT).
