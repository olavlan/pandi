...
-opaque test_spec(DVK) :: {timeout, integer(), fun(() -> DVK)}.

-opaque test_group(DVL) :: {inparallel, list(test_spec(DVL))} |
    {inorder, list(test_spec(DVL))}.
...
-spec make(fun(() -> DVM)) -> test_spec(DVM).
make(F) ->
...
-spec make_with_timeout(integer(), fun(() -> DVO)) -> test_spec(DVO).
...
-spec run_in_parallel(list(test_spec(DVQ))) -> test_group(DVQ).
...
-spec run_in_order(list(test_spec(DVU))) -> test_group(DVU).
