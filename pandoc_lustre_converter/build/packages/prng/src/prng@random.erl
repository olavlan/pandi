...
-export([new_seed/1, step/2, int/2, float/2, constant/1, fixed_size_list/2, then/2, pair/2, list/1, map/2, weighted/2, uniform/2, try_uniform/1, try_weighted/1, choose/2, map2/3, map3/4, map4/5, map5/6, shuffle/1, fixed_size_string/1, string/0, bit_array/0, set/1, fixed_size_set/2, dict/2, fixed_size_dict/3, sample/2]).
...
-opaque generator(DQM) :: {generator, fun((seed()) -> {DQM, seed()})}.
...
-spec step(generator(DQN), seed()) -> {DQN, seed()}.
step(Generator, Seed) ->
...
-spec constant(DQR) -> generator(DQR).
constant(Value) ->
...
-spec get_by_weight({float(), DRL}, list({float(), DRL}), float()) -> DRL.
...
-spec do_fixed_size_list(list(DRY), seed(), generator(DRY), integer()) -> {list(DRY),
    seed()}.
...
-spec fixed_size_list(generator(DRU), integer()) -> generator(list(DRU)).
...
-spec then(generator(DUU), fun((DUU) -> generator(DUW))) -> generator(DUW).
...
-spec pair(generator(DRP), generator(DRR)) -> generator({DRP, DRR}).
pair(One, Other) ->
...
-spec list(generator(DSC)) -> generator(list(DSC)).
list(Generator) ->
...
-spec map(generator(DUZ), fun((DUZ) -> DVB)) -> generator(DVB).
map(Generator, Fun) ->
    {generator,
        fun(Seed) ->
            {Value, Seed@1} = step(Generator, Seed),
...
-spec weighted({float(), DRB}, list({float(), DRB})) -> generator(DRB).
...
-spec uniform(DQT, list(DQT)) -> generator(DQT).
...
-spec try_uniform(list(DQW)) -> {ok, generator(DQW)} | {error, nil}.
try_uniform(Options) ->
    case Options of
        [First | Rest] ->
...
-spec try_weighted(list({float(), DRG})) -> {ok, generator(DRG)} | {error, nil}.
...
-spec choose(DRN, DRN) -> generator(DRN).
choose(One, Other) ->
    uniform(One, [Other]).
...
-spec map2(generator(DVD), generator(DVF), fun((DVD, DVF) -> DVH)) -> generator(DVH).
map2(One, Other, Fun) ->
    {generator,
        fun(Seed) ->
...
-spec do_shuffle(list(DTR), list({float(), DTR})) -> generator(list(DTR)).
do_shuffle(List, Acc) ->
...
-spec shuffle(list(DTN)) -> generator(list(DTN)).
shuffle(List) ->
    do_shuffle(List, []).
...
-spec set(generator(DTI)) -> generator(gleam@set:set(DTI)).
set(Generator) ->
...
-spec fixed_size_set(generator(DSZ), integer()) -> generator(gleam@set:set(DSZ)).
...
-spec fixed_size_dict(generator(DSG), generator(DSI), integer()) -> generator(gleam@dict:dict(DSG, DSI)).
...
-spec do_fixed_size_dict(
    ...
    integer(),
    integer(),
    gleam@dict:dict(DSL, DSN)
) -> generator(gleam@dict:dict(DSL, DSN)).
...
-spec sample_loop(
    list(DUA),
    gleam@dict:dict(integer(), DUA),
    integer(),
    float()
) -> generator(gleam@dict:dict(integer(), DUA)).
...
-spec build_reservoir_loop(
    list(DUN),
    integer(),
    gleam@dict:dict(integer(), DUN)
) -> {gleam@dict:dict(integer(), DUN), list(DUN)}.
...
-spec build_reservoir(list(DUI), integer()) -> {gleam@dict:dict(integer(), DUI),
    list(DUI)}.
...
-spec sample(list(DTW), integer()) -> generator(list(DTW)).
sample(List, N) ->
