-module(render_inline).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch, inline]).
-define(FILEPATH, "test/render_inline.gleam").
-export([str_test/0, space_test/0, line_break_test/0, soft_break_test/0, emph_test/0, strong_test/0, strikeout_test/0, code_test/0, code_with_attributes_test/0, span_test/0, span_with_attributes_test/0, link_test/0, link_with_title_test/0, link_with_attributes_test/0]).

-file("test/render_inline.gleam", 6).
-spec snapshot(pandi@pandoc:inline(), binary()) -> nil.
snapshot(Inline, Title) ->
    _pipe = pandoc_lustre_converter:convert_inlines([Inline]),
    _pipe@1 = lustre@element:to_readable_string(_pipe),
    birdie:snap(_pipe@1, <<"[render_inline] "/utf8, Title/binary>>).

-file("test/render_inline.gleam", 12).
-spec str_test() -> nil.
str_test() ->
    _pipe = {str, <<"Hello"/utf8>>},
    snapshot(_pipe, <<"simple string"/utf8>>).

-file("test/render_inline.gleam", 17).
-spec space_test() -> nil.
space_test() ->
    _pipe = space,
    snapshot(_pipe, <<"space"/utf8>>).

-file("test/render_inline.gleam", 22).
-spec line_break_test() -> nil.
line_break_test() ->
    _pipe = line_break,
    snapshot(_pipe, <<"line break"/utf8>>).

-file("test/render_inline.gleam", 27).
-spec soft_break_test() -> nil.
soft_break_test() ->
    _pipe = soft_break,
    snapshot(_pipe, <<"soft break"/utf8>>).

-file("test/render_inline.gleam", 32).
-spec emph_test() -> nil.
emph_test() ->
    _pipe = {emph, [{str, <<"Emphasized"/utf8>>}]},
    snapshot(_pipe, <<"emphasis"/utf8>>).

-file("test/render_inline.gleam", 37).
-spec strong_test() -> nil.
strong_test() ->
    _pipe = {strong, [{str, <<"Bold"/utf8>>}]},
    snapshot(_pipe, <<"strong"/utf8>>).

-file("test/render_inline.gleam", 42).
-spec strikeout_test() -> nil.
strikeout_test() ->
    _pipe = {strikeout, [{str, <<"Deleted"/utf8>>}]},
    snapshot(_pipe, <<"strikeout"/utf8>>).

-file("test/render_inline.gleam", 47).
-spec code_test() -> nil.
code_test() ->
    _pipe = {code, {attributes, <<""/utf8>>, [], []}, <<"let x = 1"/utf8>>},
    snapshot(_pipe, <<"inline code without attributes"/utf8>>).

-file("test/render_inline.gleam", 52).
...
-file("test/render_inline.gleam", 59).
-spec span_test() -> nil.
span_test() ->
    _pipe = {span,
        {attributes, <<""/utf8>>, [], []},
...
-file("test/render_inline.gleam", 64).
...
-file("test/render_inline.gleam", 70).
-spec link_test() -> nil.
link_test() ->
...
-file("test/render_inline.gleam", 76).
...
-file("test/render_inline.gleam", 82).
