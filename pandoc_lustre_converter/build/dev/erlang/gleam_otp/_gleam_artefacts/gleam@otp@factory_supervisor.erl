...
-opaque supervisor(OII, OIJ) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(OII, OIJ))}.

-type message(OIK, OIL) :: any() | {gleam_phantom, OIK, OIL}.

-opaque builder(OIM, OIN) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((OIM) -> {ok, gleam@otp@actor:started(OIN)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(OIM, OIN)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(OIO, OIP) :: {local,
        gleam@erlang@process:name(message(OIO, OIP))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(OIQ) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, OIQ}.
...
-type erlang_child_spec_property(OIR, OIS) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((OIR) -> {ok, gleam@otp@actor:started(OIS)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(OIT, OIU))) -> supervisor(OIT, OIU).
get_by_name(Name) ->
...
-spec worker_child(
    fun((OJA) -> {ok, gleam@otp@actor:started(OJB)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(OJA, OJB).
...
-spec supervisor_child(
    fun((OJF) -> {ok, gleam@otp@actor:started(OJG)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(OJF, OJG).
...
-spec named(builder(OJK, OJL), gleam@erlang@process:name(message(OJK, OJL))) -> builder(OJK, OJL).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(OJT, OJU), integer(), integer()) -> builder(OJT, OJU).
...
-spec timeout(builder(OJZ, OKA), integer()) -> builder(OJZ, OKA).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(OKF, OKG), gleam@otp@supervision:restart()) -> builder(OKF, OKG).
...
-spec start(builder(OKL, OKM)) -> {ok,
        gleam@otp@actor:started(supervisor(OKL, OKM))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(OLK, OLL)) -> gleam@otp@supervision:child_specification(supervisor(OLK, OLL)).
supervised(Builder) ->
...
-spec start_child(supervisor(OLR, OLS), OLR) -> {ok,
        gleam@otp@actor:started(OLS)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((OMP) -> {ok, gleam@otp@actor:started(OMQ)} |
        {error, gleam@otp@actor:start_error()}),
    OMP
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), OMQ, gleam@otp@actor:start_error()).
