...
-type child_specification(OFU) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(OFU)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(OFV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(OFV).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(OGA)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(OGA).
supervisor(Start) ->
...
-spec significant(child_specification(OGF), boolean()) -> child_specification(OGF).
...
-spec timeout(child_specification(OGI), integer()) -> child_specification(OGI).
timeout(Child, Ms) ->
...
-spec restart(child_specification(OGL), restart()) -> child_specification(OGL).
...
-spec map_data(child_specification(OGO), fun((OGO) -> OGQ)) -> child_specification(OGQ).
