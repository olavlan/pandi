...
-type message(NMP) :: {message, NMP} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(NMQ, NMR) :: {continue,
        NMQ,
        gleam@option:option(gleam@erlang@process:selector(NMR))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(NMS, NMT) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        NMS,
        gleam@erlang@process:selector(message(NMT)),
        gleam@otp@system:debug_state(),
        fun((NMS, NMT) -> next(NMS, NMT))}.

-type started(NMU) :: {started, gleam@erlang@process:pid_(), NMU}.

-opaque initialised(NMV, NMW, NMX) :: {initialised,
        NMV,
        gleam@option:option(gleam@erlang@process:selector(NMW)),
        NMX}.

-opaque builder(NMY, NMZ, NNA) :: {builder,
        fun((gleam@erlang@process:subject(NMZ)) -> {ok,
                initialised(NMY, NMZ, NNA)} |
            {error, binary()}),
        integer(),
        fun((NMY, NMZ) -> next(NMY, NMZ)),
        gleam@option:option(gleam@erlang@process:name(NMZ))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(NNB) :: {ack, {ok, NNB} | {error, binary()}} |
...
-spec continue(NNG) -> next(NNG, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(NNS, NNT), gleam@erlang@process:selector(NNT)) -> next(NNS, NNT).
...
-spec initialised(NNZ) -> initialised(NNZ, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(NOE, any(), NOG),
    gleam@erlang@process:selector(NOK)
) -> initialised(NOE, NOK, NOG).
...
-spec returning(initialised(NOP, NOQ, any()), NOV) -> initialised(NOP, NOQ, NOV).
...
-spec new(NOZ) -> builder(NOZ, NPA, gleam@erlang@process:subject(NPA)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(NPF)) -> {ok, initialised(NPH, NPF, NPI)} |
        {error, binary()})
) -> builder(NPH, NPF, NPI).
...
-spec on_message(builder(NPR, NPS, NPT), fun((NPR, NPS) -> next(NPR, NPS))) -> builder(NPR, NPS, NPT).
...
-spec named(builder(NQC, NQD, NQE), gleam@erlang@process:name(NQD)) -> builder(NQC, NQD, NQE).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(NQR))) -> gleam@erlang@process:selector(message(NQR)).
...
-spec receive_message(self(any(), NQN)) -> message(NQN).
...
-spec initialise_actor(
    builder(any(), any(), NRK),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, NRK} | {error, binary()})
...
-spec start(builder(any(), any(), NRX)) -> {ok, started(NRX)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(NSE), NSE) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(NSG),
    integer(),
    fun((gleam@erlang@process:subject(NSI)) -> NSG)
) -> NSI.
