...
-opaque subject(NDE) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(NDE)}.

-type name(NDF) :: any() | {gleam_phantom, NDF}.

-type do_not_leak() :: any().

-type selector(NDG) :: any() | {gleam_phantom, NDG}.
...
-spec 'receive'(subject(NEE), integer()) -> {ok, NEE} | {error, nil}.
...
-spec receive_forever(subject(NEM)) -> NEM.
...
-spec selector_receive(selector(NEQ), integer()) -> {ok, NEQ} | {error, nil}.
...
-spec selector_receive_forever(selector(NEU)) -> NEU.
...
-spec map_selector(selector(NEW), fun((NEW) -> NEY)) -> selector(NEY).
map_selector(A, B) ->
...
-spec merge_selector(selector(NFA), selector(NFA)) -> selector(NFA).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(NFE), fun((exit_message()) -> NFE)) -> selector(NFE).
...
-spec select_map(selector(NFL), subject(NFN), fun((NFN) -> NFL)) -> selector(NFL).
...
-spec select(selector(NFH), subject(NFH)) -> selector(NFH).
...
-spec deselect(selector(NFQ), subject(any())) -> selector(NFQ).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> NFV)
) -> selector(NFV).
...
-spec select_other(selector(NFZ), fun((gleam@dynamic:dynamic_()) -> NFZ)) -> selector(NFZ).
...
-spec select_specific_monitor(selector(NGL), monitor(), fun((down()) -> NGL)) -> selector(NGL).
...
-spec select_monitors(selector(NGO), fun((down()) -> NGO)) -> selector(NGO).
...
-spec deselect_specific_monitor(selector(NGR), monitor()) -> selector(NGR).
...
-spec perform_call(
    subject(NGU),
    fun((subject(NGW)) -> NGU),
    fun((selector(NGW)) -> {ok, NGW} | {error, nil})
) -> NGW.
...
-spec call(subject(NHB), integer(), fun((subject(NHD)) -> NHB)) -> NHD.
...
-spec call_forever(subject(NHF), fun((subject(NHH)) -> NHF)) -> NHH.
...
-spec send_after(subject(NHN), integer(), NHN) -> timer().
