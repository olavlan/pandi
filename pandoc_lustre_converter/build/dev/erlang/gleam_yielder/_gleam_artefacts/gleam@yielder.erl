...
-type action(OXN) :: stop | {continue, OXN, fun(() -> action(OXN))}.

-opaque yielder(OXO) :: {yielder, fun(() -> action(OXO))}.

-type step(OXP, OXQ) :: {next, OXP, OXQ} | done.

-type chunk(OXR, OXS) :: {another_by,
        list(OXR),
        OXS,
        OXR,
        fun(() -> action(OXR))} |
...
-type sized_chunk(OXT) :: {another, list(OXT), fun(() -> action(OXT))} |
    {last, list(OXT)} |
    no_more.
...
-spec unfold_loop(OYB, fun((OYB) -> step(OYC, OYB))) -> fun(() -> action(OYC)).
...
-spec unfold(OXW, fun((OXW) -> step(OXX, OXW))) -> yielder(OXX).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> OYG)) -> yielder(OYG).
repeatedly(F) ->
...
-spec from_list(list(OYK)) -> yielder(OYK).
from_list(List) ->
...
-spec transform(yielder(OYU), OYW, fun((OYW, OYU) -> step(OYX, OYW))) -> yielder(OYX).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(OZE)), fun((OZG, OZE) -> OZG), OZG) -> OZG.
...
-spec fold(yielder(OZB), OZD, fun((OZD, OZB) -> OZD)) -> OZD.
...
-spec to_list(yielder(OZJ)) -> list(OZJ).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(OZM)) -> step(OZM, yielder(OZM)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(OZU)), integer()) -> fun(() -> action(OZU)).
...
-spec take(yielder(OZR), integer()) -> yielder(OZR).
...
-spec drop_loop(fun(() -> action(PAA)), integer()) -> action(PAA).
...
-spec map(yielder(PAD), fun((PAD) -> PAF)) -> yielder(PAF).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(PAL), yielder(PAN), fun((PAL, PAN) -> PAP)) -> yielder(PAP).
...
-spec append_loop(fun(() -> action(PBB)), fun(() -> action(PBB))) -> action(PBB).
...
-spec append(yielder(PAX), yielder(PAX)) -> yielder(PAX).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(PBJ)))) -> action(PBJ).
...
-spec flatten(yielder(yielder(PBF))) -> yielder(PBF).
flatten(Yielder) ->
...
-spec concat(list(yielder(PBN))) -> yielder(PBN).
concat(Yielders) ->
...
-spec flat_map(yielder(PBR), fun((PBR) -> yielder(PBT))) -> yielder(PBT).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(PBZ)), fun((PBZ) -> boolean())) -> action(PBZ).
...
-spec filter(yielder(PBW), fun((PBW) -> boolean())) -> yielder(PBW).
...
-spec filter_map(yielder(PCC), fun((PCC) -> {ok, PCE} | {error, any()})) -> yielder(PCE).
...
-spec cycle(yielder(PCQ)) -> yielder(PCQ).
cycle(Yielder) ->
...
-spec find_loop(fun(() -> action(PCY)), fun((PCY) -> boolean())) -> {ok, PCY} |
    {error, nil}.
...
-spec find(yielder(PCU), fun((PCU) -> boolean())) -> {ok, PCU} | {error, nil}.
...
-spec find_map(yielder(PDC), fun((PDC) -> {ok, PDE} | {error, any()})) -> {ok,
        PDE} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(PDV)), integer()) -> fun(() -> action({PDV,
    integer()})).
...
-spec index(yielder(PDS)) -> yielder({PDS, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(PDY, fun((PDY) -> PDY)) -> yielder(PDY).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(PED)), fun((PED) -> boolean())) -> fun(() -> action(PED)).
...
-spec take_while(yielder(PEA), fun((PEA) -> boolean())) -> yielder(PEA).
...
-spec drop_while_loop(fun(() -> action(PEJ)), fun((PEJ) -> boolean())) -> action(PEJ).
...
-spec drop_while(yielder(PEG), fun((PEG) -> boolean())) -> yielder(PEG).
...
-spec scan_loop(fun(() -> action(PEQ)), fun((PES, PEQ) -> PES), PES) -> fun(() -> action(PES)).
...
-spec scan(yielder(PEM), PEO, fun((PEO, PEM) -> PEO)) -> yielder(PEO).
...
-spec zip_loop(fun(() -> action(PEZ)), fun(() -> action(PFB))) -> fun(() -> action({PEZ,
    PFB})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(PEU), yielder(PEW)) -> yielder({PEU, PEW}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(PFO)), fun((PFO) -> PFQ), PFQ, list(PFO)) -> chunk(PFO, PFQ).
...
-spec chunk_loop(fun(() -> action(PFJ)), fun((PFJ) -> PFL), PFL, PFJ) -> action(list(PFJ)).
...
-spec chunk(yielder(PFE), fun((PFE) -> any())) -> yielder(list(PFE)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(PGC)), integer(), list(PGC)) -> sized_chunk(PGC).
...
-spec sized_chunk_loop(fun(() -> action(PFY)), integer()) -> fun(() -> action(list(PFY))).
...
-spec sized_chunk(yielder(PFU), integer()) -> yielder(list(PFU)).
...
-spec intersperse_loop(fun(() -> action(PGJ)), PGJ) -> action(PGJ).
...
-spec intersperse(yielder(PGG), PGG) -> yielder(PGG).
...
-spec any_loop(fun(() -> action(PGO)), fun((PGO) -> boolean())) -> boolean().
...
-spec any(yielder(PGM), fun((PGM) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(PGS)), fun((PGS) -> boolean())) -> boolean().
...
-spec all(yielder(PGQ), fun((PGQ) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(PHI) -> fun((gleam@option:option(list(PHI))) -> list(PHI)).
...
-spec group_updater(fun((PHA) -> PHB)) -> fun((gleam@dict:dict(PHB, list(PHA)), PHA) -> gleam@dict:dict(PHB, list(PHA))).
group_updater(F) ->
...
-spec group(yielder(PGU), fun((PGU) -> PGW)) -> gleam@dict:dict(PGW, list(PGU)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(PHM), fun((PHM, PHM) -> PHM)) -> {ok, PHM} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(PHQ)) -> {ok, PHQ} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(PHY) -> yielder(PHY).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(PIE)), fun(() -> action(PIE))) -> action(PIE).
...
-spec interleave(yielder(PIA), yielder(PIA)) -> yielder(PIA).
...
-spec fold_until_loop(
    fun(() -> action(PIM)),
    fun((PIO, PIM) -> gleam@list:continue_or_stop(PIO)),
    PIO
) -> PIO.
...
-spec fold_until(
    yielder(PII),
    PIK,
    fun((PIK, PII) -> gleam@list:continue_or_stop(PIK))
) -> PIK.
...
-spec try_fold(yielder(PIQ), PIS, fun((PIS, PIQ) -> {ok, PIS} | {error, PIT})) -> {ok,
        PIS} |
    {error, PIT}.
...
-spec first(yielder(PJG)) -> {ok, PJG} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(PJK), integer()) -> {ok, PJK} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(PJS), fun((PJS) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(PJV, fun(() -> yielder(PJV))) -> yielder(PJV).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(PJY), PJY) -> yielder(PJY).
