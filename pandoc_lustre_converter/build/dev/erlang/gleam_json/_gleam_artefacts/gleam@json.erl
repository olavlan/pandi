...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(GDX)) -> {ok, GDX} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(GDR)) -> {ok, GDR} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(GDN)) -> {ok, GDN} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(GED), fun((GED) -> json())) -> json().
...
-spec array(list(GEH), fun((GEH) -> json())) -> json().
