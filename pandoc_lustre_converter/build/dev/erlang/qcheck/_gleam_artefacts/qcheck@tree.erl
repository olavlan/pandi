...
-export([new/2, map/2, bind/2, apply/2, return/1, map2/3, sequence_trees/1, option/1, collect/2, to_string/2, to_string_max_depth/3]).
-export_type([tree/1]).
...
-type tree(ABSM) :: {tree, ABSM, gleam@yielder:yielder(tree(ABSM))}.

-file("src/qcheck/tree.gleam", 39).
-spec new(ABSN, fun((ABSN) -> gleam@yielder:yielder(ABSN))) -> tree(ABSN).
new(X, Shrink) ->
    Shrink_trees = begin
        _pipe = Shrink(X),
...
-spec map(tree(ABSQ), fun((ABSQ) -> ABSS)) -> tree(ABSS).
map(Tree, F) ->
    {tree, X, Xs} = Tree,
    Y = F(X),
...
-spec bind(tree(ABSU), fun((ABSU) -> tree(ABSW))) -> tree(ABSW).
bind(Tree, F) ->
    {tree, X, Xs} = Tree,
    {tree, Y, Ys_of_x} = F(X),
...
-spec apply(tree(fun((ABSZ) -> ABTA)), tree(ABSZ)) -> tree(ABTA).
apply(F, X) ->
    {tree, X0, Xs} = X,
    {tree, F0, Fs} = F,
    Y = F0(X0),
...
-spec return(ABTE) -> tree(ABTE).
return(X) ->
    {tree, X, gleam@yielder:empty()}.

-file("src/qcheck/tree.gleam", 192).
-spec parameter(fun((ABUN) -> ABUO)) -> fun((ABUN) -> ABUO).
parameter(F) ->
    F.

-file("src/qcheck/tree.gleam", 86).
-spec map2(tree(ABTG), tree(ABTI), fun((ABTG, ABTI) -> ABTK)) -> tree(ABTK).
map2(A, B, F) ->
    _pipe = begin
...
-spec yielder_cons(ABTR, fun(() -> gleam@yielder:yielder(ABTR))) -> gleam@yielder:yielder(ABTR).
yielder_cons(Element, Yielder) ->
    gleam@yielder:yield(Element, Yielder).

-file("src/qcheck/tree.gleam", 112).
-spec option(tree(ABTU)) -> tree(gleam@option:option(ABTU)).
option(Tree) ->
    {tree, X, Xs} = Tree,
...
-spec do_collect(tree(ABUC), fun((ABUC) -> ABUE), list(ABUE)) -> list(ABUE).
...
-spec collect(tree(ABTY), fun((ABTY) -> ABUA)) -> list(ABUA).
collect(Tree, F) ->
    do_collect(Tree, F, []).

-file("src/qcheck/tree.gleam", 159).
-spec do_to_string(tree(ABUL), fun((ABUL) -> binary()), integer(), integer()) -> binary().
...
-spec to_string(tree(ABUH), fun((ABUH) -> binary())) -> binary().
...
-spec to_string_max_depth(tree(ABUJ), fun((ABUJ) -> binary()), integer()) -> binary().
