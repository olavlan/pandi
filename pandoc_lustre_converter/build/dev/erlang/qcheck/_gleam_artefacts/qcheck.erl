...
-export([with_seed/2, run/3, random_seed/0, default_config/0, given/2, seed/1, with_max_retries/2, with_test_count/2, config/3, generator/2, generate/3, generate_tree/2, return/1, constant/1, parameter/1, map/2, bind/2, then/2, apply/2, map2/3, map3/4, map4/5, map5/6, map6/7, tuple2/2, tuple3/3, tuple4/4, tuple5/5, tuple6/6, from_generators/2, from_weighted_generators/2, sized_from/2, small_non_negative_int/0, small_strictly_positive_int/0, bounded_int/2, uniform_int/0, float/0, bounded_float/2, uniform_codepoint/0, uniform_printable_ascii_codepoint/0, ascii_digit_codepoint/0, lowercase_ascii_codepoint/0, uppercase_ascii_codepoint/0, codepoint/0, fixed_length_string_from/2, string/0, bounded_codepoint/2, alphabetic_ascii_codepoint/0, alphanumeric_ascii_codepoint/0, printable_ascii_codepoint/0, codepoint_from_ints/2, codepoint_from_strings/2, ascii_whitespace_codepoint/0, generic_string/2, non_empty_string/0, string_from/1, non_empty_string_from/1, fixed_length_list_from/2, generic_list/2, list_from/1, generic_dict/3, generic_set/2, option_from/1, nil/0, bool/0, fixed_size_bit_array_from/2, generic_bit_array/2, bit_array/0, non_empty_bit_array/0, fixed_size_bit_array/1, utf8_bit_array/0, non_empty_utf8_bit_array/0, fixed_size_utf8_bit_array/1, fixed_size_utf8_bit_array_from/2, generic_utf8_bit_array/2, byte_aligned_bit_array/0, non_empty_byte_aligned_bit_array/0, fixed_size_byte_aligned_bit_array/1, fixed_size_byte_aligned_bit_array_from/2, generic_byte_aligned_bit_array/2]).
...
-type generator(ABXY) :: {generator,
        fun((qcheck@random:seed()) -> {qcheck@tree:tree(ABXY),
            qcheck@random:seed()})}.
...
-type 'try'(ABXZ) :: {no_panic, ABXZ} | {panic, exception:exception()}.

-file("src/qcheck.gleam", 2799).
-spec failwith(ACJB, ACJB, integer(), exception:exception()) -> any().
...
-spec do_run_property(fun((ABYJ) -> nil), ABYJ, integer(), integer()) -> run_property_result().
...
-spec run_property(fun((ABYI) -> nil), ABYI, integer()) -> run_property_result().
run_property(Property, Value, Max_retries) ->
    do_run_property(Property, Value, Max_retries, 0).

-file("src/qcheck.gleam", 408).
-spec do_filter_map(
    gleam@yielder:yielder(ABYT),
    fun((ABYT) -> gleam@option:option(ABYV))
) -> gleam@yielder:step(ABYV, gleam@yielder:yielder(ABYT)).
do_filter_map(It, F) ->
...
-spec filter_map(
    gleam@yielder:yielder(ABYO),
    fun((ABYO) -> gleam@option:option(ABYQ))
) -> gleam@yielder:yielder(ABYQ).
filter_map(It, F) ->
    gleam@yielder:unfold(It, fun(_capture) -> do_filter_map(_capture, F) end).

-file("src/qcheck.gleam", 372).
-spec do_shrink(
    qcheck@tree:tree(ABYM),
    fun((ABYM) -> nil),
    integer(),
    integer()
) -> {ABYM, integer()}.
...
-spec shrink(qcheck@tree:tree(ABYK), fun((ABYK) -> nil), integer()) -> {ABYK,
    integer()}.
...
-spec do_run(config(), generator(ABYG), fun((ABYG) -> nil), integer()) -> nil.
...
-spec run(config(), generator(ABYC), fun((ABYC) -> nil)) -> nil.
...
-spec given(generator(ABYE), fun((ABYE) -> nil)) -> nil.
...
-spec generator(
    qcheck@random:generator(ABZA),
    fun((ABZA) -> qcheck@tree:tree(ABZA))
) -> generator(ABZA).
...
-spec generate(generator(ABZE), integer(), qcheck@random:seed()) -> {list(ABZE),
    qcheck@random:seed()}.
...
-spec generate_tree(generator(ABZL), qcheck@random:seed()) -> {qcheck@tree:tree(ABZL),
    qcheck@random:seed()}.
...
-spec return(ABZO) -> generator(ABZO).
return(A) ->
...
-spec constant(ABZQ) -> generator(ABZQ).
constant(A) ->
    return(A).
...
-spec parameter(fun((ABZS) -> ABZT)) -> fun((ABZS) -> ABZT).
parameter(F) ->
    F.
...
-spec map(generator(ABZU), fun((ABZU) -> ABZW)) -> generator(ABZW).
map(Generator, F) ->
    {generator, Generate} = Generator,
    {generator,
        fun(Seed) ->
...
-spec bind(generator(ABZY), fun((ABZY) -> generator(ACAA))) -> generator(ACAA).
bind(Generator, F) ->
    {generator, Generate} = Generator,
    {generator,
        fun(Seed) ->
...
-spec then(generator(ACAD), fun((ACAD) -> generator(ACAF))) -> generator(ACAF).
then(Generator, F) ->
    bind(Generator, F).
...
-spec apply(generator(fun((ACAI) -> ACAJ)), generator(ACAI)) -> generator(ACAJ).
apply(F, X) ->
    {generator, F@1} = F,
    {generator, X@1} = X,
    {generator,
...
-spec map2(generator(ACAN), generator(ACAP), fun((ACAN, ACAP) -> ACAR)) -> generator(ACAR).
map2(G1, G2, F) ->
    _pipe = return(
        begin
...
-spec map5(
    ...
    generator(ACBR),
    generator(ACBT),
    fun((ACBL, ACBN, ACBP, ACBR, ACBT) -> ACBV)
) -> generator(ACBV).
...
-spec map6(
    ...
    generator(ACCF),
    generator(ACCH),
    fun((ACBX, ACBZ, ACCB, ACCD, ACCF, ACCH) -> ACCJ)
) -> generator(ACCJ).
...
-spec tuple2(generator(ACCL), generator(ACCN)) -> generator({ACCL, ACCN}).
tuple2(G1, G2) ->
...
-spec tuple3(generator(ACCQ), generator(ACCS), generator(ACCU)) -> generator({ACCQ,
    ACCS,
    ACCU}).
tuple3(G1, G2, G3) ->
...
-spec tuple4(generator(ACCX), generator(ACCZ), generator(ACDB), generator(ACDD)) -> generator({ACCX,
    ACCZ,
    ACDB,
    ACDD}).
tuple4(G1, G2, G3, G4) ->
...
-spec tuple5(
    ...
    generator(ACDM),
    generator(ACDO)
) -> generator({ACDG, ACDI, ACDK, ACDM, ACDO}).
...
-spec tuple6(
    ...
    generator(ACDZ),
    generator(ACEB)
) -> generator({ACDR, ACDT, ACDV, ACDX, ACDZ, ACEB}).
...
-spec from_generators(generator(ACEE), list(generator(ACEE))) -> generator(ACEE).
...
-spec from_weighted_generators(
    {integer(), generator(ACEJ)},
    list({integer(), generator(ACEJ)})
) -> generator(ACEJ).
...
-spec sized_from(fun((integer()) -> generator(ACEO)), generator(integer())) -> generator(ACEO).
...
-spec list_cons(ACJF, list(ACJF)) -> list(ACJF).
list_cons(X, Xs) ->
    [X | Xs].
...
-spec generic_list_loop(
    integer(),
    qcheck@tree:tree(list(ACGF)),
    generator(ACGF),
    qcheck@random:seed()
) -> {qcheck@tree:tree(list(ACGF)), qcheck@random:seed()}.
...
-spec fixed_length_list_from(generator(ACGQ), integer()) -> generator(list(ACGQ)).
...
-spec generic_list(generator(ACGL), generator(integer())) -> generator(list(ACGL)).
...
-spec list_from(generator(ACGU)) -> generator(list(ACGU)).
...
-spec generic_dict(generator(ACGY), generator(ACHA), generator(integer())) -> generator(gleam@dict:dict(ACGY, ACHA)).
...
-spec generic_set(generator(ACHG), generator(integer())) -> generator(gleam@set:set(ACHG)).
...
-spec option_from(generator(ACHM)) -> generator(gleam@option:option(ACHM)).
...
value_with_size_list_to_bit_array(Value_with_size_list) ->
    gleam@list:fold(
        ...
        fun(Acc, _use1) ->
            ...
            <<Int:(erlang:max(0, Size)), Acc/bitstring>>
        end
    ).
    ...
    case K of
        ...
        K@2 when K@2 =< 8 ->
            ...
            do_gen_bit_array(
                Value_generator,
                Seed@1,
                <<Root:(erlang:max(0, K@2)), Acc/bitstring>>,
