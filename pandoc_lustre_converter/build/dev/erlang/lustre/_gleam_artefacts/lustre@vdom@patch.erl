...
-type patch(TNU) :: {patch,
        integer(),
        integer(),
        list(change(TNU)),
        list(patch(TNU))}.

-type change(TNV) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(TNV)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(TNV)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(TNW)), list(patch(TNW))) -> patch(TNW).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(TOG)),
    list(lustre@vdom@vattr:attribute(TOG))
) -> change(TOG).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(TOQ)) -> change(TOQ).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(TOT)), integer()) -> change(TOT).
...
?DOC(false).
-spec add_child(patch(TOZ), patch(TOZ)) -> patch(TOZ).
...
-spec insert_to_json(
    ...
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TRQ)), lustre@vdom@vnode:element(TRQ))
) -> gleam@json:json().
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(TRS),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TRS)), lustre@vdom@vnode:element(TRS))
) -> gleam@json:json().
...
-spec change_to_json(
    change(TPG),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TPG)), lustre@vdom@vnode:element(TPG))
) -> gleam@json:json().
...
-spec to_json(
    patch(TPD),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TPD)), lustre@vdom@vnode:element(TPD))
) -> gleam@json:json().
to_json(Patch, Memos) ->
