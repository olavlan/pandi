...
-opaque cache(TYV) :: {cache,
        events(TYV),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TYV)), lustre@vdom@vnode:element(TYV)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TYV)), lustre@vdom@vnode:element(TYV)),
        list(binary()),
        list(binary())}.

-opaque events(TYW) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(TYW))),
        lustre@internals@mutable_map:mutable_map(binary(), child(TYW))}.

-type child(TYX) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(TYX)}.

-type added_children(TYY) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(TYY))),
        lustre@internals@mutable_map:mutable_map(binary(), child(TYY)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TYY)), lustre@vdom@vnode:element(TYY))}.

-opaque decoded_event(TYZ) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(TYZ)} |
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UAV))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UAV))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UAV))).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UBY))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(UBY))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UBY))).
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UCQ))),
    lustre@internals@mutable_map:mutable_map(binary(), child(UCQ)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UCQ)), lustre@vdom@vnode:element(UCQ)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UCQ))
) -> added_children(UCQ).
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UCJ))
) -> {cache(UCJ), events(UCJ)}.
...
-spec add_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(UBS)
) -> {cache(UBS), events(UBS)}.
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(TZE)) -> cache(TZE).
from_node(Root) ->
    Cache = new(),
...
?DOC(false).
-spec events(cache(TZK)) -> events(TZK).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(TZN), events(TZN)) -> cache(TZN).
...
?DOC(false).
-spec memos(cache(TZR)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TZR)), lustre@vdom@vnode:element(TZR)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(TZU),
    fun(() -> lustre@vdom@vnode:element(TZU)),
    fun(() -> lustre@vdom@vnode:element(TZU))
) -> lustre@vdom@vnode:element(TZU).
...
-spec keep_memo(
    cache(TZZ),
    fun(() -> lustre@vdom@vnode:element(TZZ)),
    fun(() -> lustre@vdom@vnode:element(TZZ))
) -> cache(TZZ).
...
-spec add_memo(
    cache(UAE),
    fun(() -> lustre@vdom@vnode:element(UAE)),
    lustre@vdom@vnode:element(UAE)
) -> cache(UAE).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UAQ))
) -> events(UAQ).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UBJ))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UBJ))).
...
?DOC(false).
-spec remove_event(events(UBG), lustre@vdom@path:path(), binary()) -> events(UBG).
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UDH))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(UDH))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UDH))).
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UDS))),
    lustre@internals@mutable_map:mutable_map(binary(), child(UDS)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UDS)), lustre@vdom@vnode:element(UDS)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UDS))
) -> events(UDS).
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(UEE),
    lustre@vdom@vnode:element(UEE)
) -> {cache(UEE), events(UEE)}.
...
?DOC(false).
-spec dispatch(cache(UES), decoded_event(UES)) -> {cache(UES),
    {ok, lustre@vdom@vattr:handler(UES)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(UEW), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(UEW),
    {ok, lustre@vdom@vattr:handler(UEW)} | {error, nil}}.
