...
-type state(VBL, VBM) :: {state,
        gleam@erlang@process:subject(message(VBM)),
        gleam@erlang@process:selector(message(VBM)),
        gleam@erlang@process:selector(message(VBM)),
        VBL,
        fun((VBL, VBM) -> {VBL, lustre@effect:effect(VBM)}),
        fun((VBL) -> lustre@vdom@vnode:element(VBM)),
        config(VBM),
        lustre@vdom@vnode:element(VBM),
        lustre@vdom@cache:cache(VBM),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(VBM)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(VBM)) -> nil))}.

-type config(VBN) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, VBN} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VBN)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VBN)),
        gleam@option:option(VBN),
        gleam@option:option(VBN)}.

-type message(VBO) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(VBO))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(VBO))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(VBO)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(VBO)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(VBO))} |
    {effect_dispatched_message, VBO} |
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(VDO)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(VDO)) -> nil)),
    lustre@runtime@transport:client_message(VDO)
) -> nil.
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(VDK)),
    lustre@effect:effect(VDK)
) -> nil.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VDE)),
    binary(),
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, VCX} | {error, nil})),
    binary(),
    binary()
) -> {ok, VCX} | {error, nil}.
...
?DOC(false).
-spec loop(state(VCH, VCI), message(VCI)) -> gleam@otp@actor:next(state(VCH, VCI), message(VCI)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(VBS))),
    fun((VBW) -> {VBX, lustre@effect:effect(VBS)}),
    fun((VBX, VBS) -> {VBX, lustre@effect:effect(VBS)}),
    fun((VBX) -> lustre@vdom@vnode:element(VBS)),
    config(VBS),
    VBW
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(VBS)))} |
