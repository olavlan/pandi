...
-type attribute(QLF) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(QLF)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(QLG) :: {handler, boolean(), boolean(), QLG}.
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(QLL)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
...
?DOC(false).
-spec merge(list(attribute(QLV)), list(attribute(QLV))) -> list(attribute(QLV)).
...
?DOC(false).
-spec compare(attribute(QMC), attribute(QMC)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(QLQ))) -> list(attribute(QLQ)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
