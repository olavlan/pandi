...
-opaque app(XYS, XYT, XYU) :: {app,
        fun((XYS) -> {XYT, lustre@effect:effect(XYU)}),
        fun((XYT, XYU) -> {XYT, lustre@effect:effect(XYU)}),
        fun((XYT) -> lustre@vdom@vnode:element(XYU))}.

-opaque simulation(XYV, XYW) :: {simulation,
        fun((XYV, XYW) -> {XYV, lustre@effect:effect(XYW)}),
        fun((XYV) -> lustre@vdom@vnode:element(XYW)),
        list(event(XYW)),
        XYV,
        lustre@vdom@vnode:element(XYW)}.

-type event(XYX) :: {dispatch, XYX} |
...
-spec simple(
    fun((XYY) -> XYZ),
    fun((XYZ, XZA) -> XYZ),
    fun((XYZ) -> lustre@vdom@vnode:element(XZA))
) -> app(XYY, XYZ, XZA).
...
-spec application(
    fun((XZF) -> {XZG, lustre@effect:effect(XZH)}),
    fun((XZG, XZH) -> {XZG, lustre@effect:effect(XZH)}),
    fun((XZG) -> lustre@vdom@vnode:element(XZH))
) -> app(XZF, XZG, XZH).
...
-spec start(app(XZO, XZP, XZQ), XZO) -> simulation(XZP, XZQ).
start(App, Args) ->
...
-spec message(simulation(XZW, XZX), XZX) -> simulation(XZW, XZX).
...
-spec problem(simulation(YBC, YBD), binary(), binary()) -> simulation(YBC, YBD).
...
-spec click(simulation(YAJ, YAK), lustre@dev@query:'query'()) -> simulation(YAJ, YAK).
...
-spec input(simulation(YAP, YAQ), lustre@dev@query:'query'(), binary()) -> simulation(YAP, YAQ).
...
-spec model(simulation(YBI, any())) -> YBI.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), YBN)) -> lustre@vdom@vnode:element(YBN).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), YBS)) -> list(event(YBS)).
history(Simulation) ->
