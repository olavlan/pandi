...
-type runtime(XAQ) :: any() | {gleam_phantom, XAQ}.
...
-spec application(
    fun((XBM) -> {XBN, lustre@effect:effect(XBO)}),
    fun((XBN, XBO) -> {XBN, lustre@effect:effect(XBO)}),
    fun((XBN) -> lustre@vdom@vnode:element(XBO))
) -> lustre@runtime@app:app(XBM, XBN, XBO).
...
-spec element(lustre@vdom@vnode:element(XAZ)) -> lustre@runtime@app:app(any(), nil, XAZ).
element(View) ->
    application(
...
-spec simple(
    fun((XBF) -> XBG),
    fun((XBG, XBH) -> XBG),
    fun((XBG) -> lustre@vdom@vnode:element(XBH))
) -> lustre@runtime@app:app(XBF, XBG, XBH).
...
-spec component(
    fun((XBV) -> {XBW, lustre@effect:effect(XBX)}),
    fun((XBW, XBX) -> {XBW, lustre@effect:effect(XBX)}),
    fun((XBW) -> lustre@vdom@vnode:element(XBX)),
    list(lustre@runtime@app:option(XBX))
) -> lustre@runtime@app:app(XBV, XBW, XBX).
...
-spec named(
    lustre@runtime@app:app(XCG, XCH, XCI),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(XCI))
) -> lustre@runtime@app:app(XCG, XCH, XCI).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(XDA, any(), XDC), binary(), XDA) -> {ok,
        runtime(XDC)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec start(lustre@runtime@app:app(XCR, any(), XCT), binary(), XCR) -> {ok,
        runtime(XCT)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(XDJ, any(), XDL), XDJ) -> {ok,
        runtime(XDL)} |
    {error, error()}.
...
-spec supervised(lustre@runtime@app:app(XDS, any(), XDU), XDS) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(XDU))).
...
-spec factory(lustre@runtime@app:app(XEB, any(), XED)) -> gleam@otp@factory_supervisor:builder(XEB, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(XED))).
factory(App) ->
...
-spec send(runtime(XES), lustre@runtime@server@runtime:message(XES)) -> nil.
...
-spec dispatch(XEV) -> lustre@runtime@server@runtime:message(XEV).
dispatch(Message) ->
