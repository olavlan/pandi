...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, WMT} | {error, nil})) -> lustre@runtime@app:option(WMT).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(WMX)) -> lustre@runtime@app:option(WMX).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(WNA)) -> lustre@runtime@app:option(WNA).
...
-spec on_form_autofill(fun((binary()) -> WNF)) -> lustre@runtime@app:option(WNF).
...
-spec on_form_reset(WNH) -> lustre@runtime@app:option(WNH).
...
-spec on_form_restore(fun((binary()) -> WNJ)) -> lustre@runtime@app:option(WNJ).
...
-spec on_connect(WNR) -> lustre@runtime@app:option(WNR).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(WNT) -> lustre@runtime@app:option(WNT).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(WNV) -> lustre@runtime@app:option(WNV).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(WNX)),
    list(lustre@vdom@vnode:element(WNX))
) -> lustre@vdom@vnode:element(WNX).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(WOD)),
    list(lustre@vdom@vnode:element(WOD))
) -> lustre@vdom@vnode:element(WOD).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), WPD),
    binary(),
    list(lustre@vdom@vattr:attribute(WPD)),
    list(lustre@vdom@vnode:element(WPD))
) -> lustre@vdom@vnode:element(WPD).
