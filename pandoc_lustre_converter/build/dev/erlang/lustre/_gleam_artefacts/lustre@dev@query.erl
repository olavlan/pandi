...
-export([element/1, child/2, descendant/2, 'and'/2, tag/1, namespaced/2, attribute/2, class/1, style/2, id/1, data/2, test_id/1, aria/2, text/1, matches/2, find_path/4, find/2, find_all/2, has/2, to_readable_string/1]).
...
-spec find_matching_in_list(
    list(lustre@vdom@vnode:element(XLE)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XLE), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_direct_child(
    lustre@vdom@vnode:element(XKZ),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XKZ), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_descendant_in_list(
    list(lustre@vdom@vnode:element(XLP)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XLP), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_descendant(
    lustre@vdom@vnode:element(XLK),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XLK), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_in_list(
    list(lustre@vdom@vnode:element(XKT)),
    'query'(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XKT), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_in_children(
    lustre@vdom@vnode:element(XKO),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XKO), lustre@vdom@path:path()}} |
    {error, nil}.
...
?DOC(false).
-spec find_path(
    lustre@vdom@vnode:element(XKJ),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XKJ), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find(lustre@vdom@vnode:element(XKE), 'query'()) -> {ok,
        lustre@vdom@vnode:element(XKE)} |
    {error, nil}.
find(Root, Query) ->
...
-spec find_all_direct_children(lustre@vdom@vnode:element(XMI), selector()) -> list(lustre@vdom@vnode:element(XMI)).
...
-spec find_all_descendants_in_list(
    list(lustre@vdom@vnode:element(XMV)),
    selector()
) -> list(lustre@vdom@vnode:element(XMV)).
...
-spec find_all_descendants(lustre@vdom@vnode:element(XMR), selector()) -> list(lustre@vdom@vnode:element(XMR)).
...
-spec find_all_in_list(list(lustre@vdom@vnode:element(XMD)), 'query'()) -> list(lustre@vdom@vnode:element(XMD)).
...
-spec find_all_in_children(lustre@vdom@vnode:element(XLZ), 'query'()) -> list(lustre@vdom@vnode:element(XLZ)).
...
-spec find_all(lustre@vdom@vnode:element(XLV), 'query'()) -> list(lustre@vdom@vnode:element(XLV)).
find_all(Root, Query) ->
    case Query of
        {find_element, Selector} ->
            case matches(Root, Selector) of
