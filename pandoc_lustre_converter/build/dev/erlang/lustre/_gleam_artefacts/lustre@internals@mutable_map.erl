...
-type mutable_map(RQQ, RQR) :: any() | {gleam_phantom, RQQ, RQR}.
...
?DOC(false).
-spec unsafe_get(mutable_map(RQW, RQX), RQW) -> RQX.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(RRE, RRF), RRE, fun(() -> RRF)) -> RRF.
...
?DOC(false).
-spec has_key(mutable_map(RRO, any()), RRO) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(RRW, RRX), RRW, RRX) -> mutable_map(RRW, RRX).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(RSI, RSJ), RSI) -> mutable_map(RSI, RSJ).
delete(Map, Key) ->
    maps:remove(Key, Map).
