...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(YJT)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(YJT)),
    list(lustre@vdom@vnode:element(YJT))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(YJT)),
    list(lustre@vdom@vnode:element(YJT))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(YJL)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(YJL)),
    list(lustre@vdom@vnode:element(YJL))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(YHR)),
    list({binary(), lustre@vdom@vnode:element(YHR)})
) -> lustre@vdom@vnode:element(YHR).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(YHX)),
    list({binary(), lustre@vdom@vnode:element(YHX)})
) -> lustre@vdom@vnode:element(YHX).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(YID)})) -> lustre@vdom@vnode:element(YID).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(YIH)),
    list({binary(), lustre@vdom@vnode:element(YIH)})
) -> lustre@vdom@vnode:element(YIH).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(YIN)),
    list({binary(), lustre@vdom@vnode:element(YIN)})
) -> lustre@vdom@vnode:element(YIN).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(YIT)),
    list({binary(), lustre@vdom@vnode:element(YIT)})
) -> lustre@vdom@vnode:element(YIT).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(YIZ)),
    list({binary(), lustre@vdom@vnode:element(YIZ)})
) -> lustre@vdom@vnode:element(YIZ).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(YJF)),
    list({binary(), lustre@vdom@vnode:element(YJF)})
) -> lustre@vdom@vnode:element(YJF).
