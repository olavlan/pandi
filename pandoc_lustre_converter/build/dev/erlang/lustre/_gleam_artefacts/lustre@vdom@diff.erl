...
-type diff(UPP) :: {diff,
        lustre@vdom@patch:patch(UPP),
        lustre@vdom@cache:cache(UPP)}.

-type partial_diff(UPQ) :: {partial_diff,
        lustre@vdom@patch:patch(UPQ),
        lustre@vdom@cache:cache(UPQ),
        lustre@vdom@cache:events(UPQ)}.

-type attribute_change(UPR) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(UPR)),
        list(lustre@vdom@vattr:attribute(UPR)),
        lustre@vdom@cache:events(UPR)}.
...
-spec diff_attributes(
    ...
    lustre@vdom@cache:events(UQU),
    list(lustre@vdom@vattr:attribute(UQU)),
    list(lustre@vdom@vattr:attribute(UQU)),
    list(lustre@vdom@vattr:attribute(UQU)),
    list(lustre@vdom@vattr:attribute(UQU))
) -> attribute_change(UQU).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(UPX)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UPX)),
    list(lustre@vdom@vnode:element(UPX)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UPX)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(UPX)),
    list(lustre@vdom@patch:patch(UPX)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(UPX),
    lustre@vdom@cache:events(UPX)
) -> partial_diff(UPX).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(UPS),
    lustre@vdom@vnode:element(UPS),
    lustre@vdom@vnode:element(UPS)
) -> diff(UPS).
diff(Cache, Old, New) ->
