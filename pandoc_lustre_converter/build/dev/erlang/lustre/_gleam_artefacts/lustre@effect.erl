...
-opaque effect(RKH) :: {effect,
        list(fun((actions(RKH)) -> nil)),
        list(fun((actions(RKH)) -> nil)),
        list(fun((actions(RKH)) -> nil))}.

-type actions(RKI) :: {actions,
        fun((RKI) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(RKI)) -> nil),
...
-spec from(fun((fun((RKL) -> nil)) -> nil)) -> effect(RKL).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((RKN) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RKN).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((RKP) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RKP).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((RKT) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(RKT))
) -> effect(RKT).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(RLA))) -> effect(RLA).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
...
-spec do_comap_select(
    actions(RLS),
    gleam@erlang@process:selector(RLU),
    fun((RLU) -> RLS)
) -> nil.
...
-spec do_comap_actions(actions(RLO), fun((RLQ) -> RLO)) -> actions(RLQ).
...
-spec do_map(list(fun((actions(RLI)) -> nil)), fun((RLI) -> RLL)) -> list(fun((actions(RLL)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(RLE), fun((RLE) -> RLG)) -> effect(RLG).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((RLW) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(RLW)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
