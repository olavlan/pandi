...
-spec on(binary(), gleam@dynamic@decode:decoder(AAIS)) -> lustre@vdom@vattr:attribute(AAIS).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(AAIV))
) -> lustre@vdom@vattr:attribute(AAIV).
...
-spec handler(AAIZ, boolean(), boolean()) -> lustre@vdom@vattr:handler(AAIZ).
...
-spec prevent_default(lustre@vdom@vattr:attribute(AAJB)) -> lustre@vdom@vattr:attribute(AAJB).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(AAJE)) -> lustre@vdom@vattr:attribute(AAJE).
...
-spec debounce(lustre@vdom@vattr:attribute(AAJH), integer()) -> lustre@vdom@vattr:attribute(AAJH).
...
-spec throttle(lustre@vdom@vattr:attribute(AAJK), integer()) -> lustre@vdom@vattr:attribute(AAJK).
...
-spec on_keypress(fun((binary()) -> AAKB)) -> lustre@vdom@vattr:attribute(AAKB).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> AAKD)) -> lustre@vdom@vattr:attribute(AAKD).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> AAKF)) -> lustre@vdom@vattr:attribute(AAKF).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> AAKH)) -> lustre@vdom@vattr:attribute(AAKH).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> AAKJ)) -> lustre@vdom@vattr:attribute(AAKJ).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> AAKL)) -> lustre@vdom@vattr:attribute(AAKL).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> AAKO)) -> lustre@vdom@vattr:attribute(AAKO).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(AAKS) -> lustre@vdom@vattr:attribute(AAKS).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(AAKU) -> lustre@vdom@vattr:attribute(AAKU).
on_blur(Msg) ->
