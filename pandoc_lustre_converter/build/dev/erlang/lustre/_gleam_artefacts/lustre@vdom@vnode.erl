...
-type element(RTY) :: {fragment,
        integer(),
        binary(),
        list(element(RTY)),
        lustre@internals@mutable_map:mutable_map(binary(), element(RTY))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(RTY)),
        list(element(RTY)),
        lustre@internals@mutable_map:mutable_map(binary(), element(RTY)),
        boolean(),
        boolean()} |
...
-spec fragment(
    binary(),
    list(element(RUG)),
    lustre@internals@mutable_map:mutable_map(binary(), element(RUG))
) -> element(RUG).
...
-spec element(
    ...
    list(element(RUN)),
    lustre@internals@mutable_map:mutable_map(binary(), element(RUN)),
    boolean(),
    boolean()
) -> element(RUN).
element(
...
-spec unsafe_inner_html(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(RUY)),
    binary()
) -> element(RUY).
...
?DOC(false).
-spec map(element(RVC), fun((RVC) -> RVE)) -> element(RVE).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(RVH))) -> element(RVH).
memo(Key, Dependencies, View) ->
    {memo, 5, Key, Dependencies, View}.

-file("src/lustre/vdom/vnode.gleam", 201).
?DOC(false).
-spec to_keyed(binary(), element(RVM)) -> element(RVM).
to_keyed(Key, Node) ->
    case Node of
...
-spec memo_to_json(
    fun(() -> element(RVP)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(RVP)), element(RVP))
) -> gleam@json:json().
