...
-spec push_key({ok, ICD} | {error, get_error()}, binary()) -> {ok, ICD} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec do(
    {ok, {IET, list(binary())}} | {error, parse_error()},
    fun((IET, list(binary())) -> {ok, IEW} | {error, parse_error()})
) -> {ok, IEW} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {IFB, list(binary())}} | {error, parse_error()})
) -> {ok, {IFB, list(binary())}} | {error, parse_error()}.
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {IEI, list(binary())}} | {error, parse_error()})
) -> {ok, {IEI, list(binary())}} | {error, parse_error()}.
