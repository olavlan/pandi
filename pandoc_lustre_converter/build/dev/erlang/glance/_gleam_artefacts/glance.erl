...
-type definition(DSJ) :: {definition, list(attribute()), DSJ}.
...
-type bit_string_segment_option(DSK) :: bytes_option |
    int_option |
    float_option |
    bits_option |
    utf8_option |
...
-type record_update_field(DSL) :: {record_update_field,
        binary(),
        gleam@option:option(DSL)}.

-type variant_field() :: {labelled_variant_field, type(), binary()} |
    {unlabelled_variant_field, type()}.

-type field(DSM) :: {labelled_field, binary(), span(), DSM} |
    {shorthand_field, binary(), span()} |
    {unlabelled_field, DSM}.
...
-type parsed_list(DSN) :: {parsed_list,
        list(DSN),
        gleam@option:option(DSN),
        list({glexer@token:token(), glexer:position()}),
        integer()}.
...
-spec list(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DZD, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    gleam@option:option(fun((span()) -> DZD)),
    list(DZD),
    list({glexer@token:token(), glexer:position()})
) -> {ok, parsed_list(DZD)} | {error, error()}.
...
-spec field(
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EBG, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {field(EBG), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
field(Tokens, Parser) ->
    case Tokens of
...
-spec comma_delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DZV, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok,
        {list(DZV), integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DXA, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list(bit_string_segment_option(DXA)),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(DXA)),
            list({glexer@token:token(), glexer:position()})}} |
...
-spec optional_bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DWT, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(DWT)),
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec bit_string_segment(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DWM, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {{DWM, list(bit_string_segment_option(DWM))},
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {DYT, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok, {list(DYT), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec expect_name(
    list({glexer@token:token(), glexer:position()}),
    fun((binary(), list({glexer@token:token(), glexer:position()})) -> {ok, DTF} |
        {error, error()})
) -> {ok, DTF} | {error, error()}.
...
-spec until(
    ...
    DTK,
    list({glexer@token:token(), glexer:position()}),
    fun((DTK, list({glexer@token:token(), glexer:position()})) -> {ok,
            {DTK, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {DTK, integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
