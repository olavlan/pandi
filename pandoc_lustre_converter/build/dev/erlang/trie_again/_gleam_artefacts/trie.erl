...
-opaque trie(GYX, GYY) :: {trie,
        gleam@option:option(GYY),
        gleam@dict:dict(GYX, trie(GYX, GYY))}.
...
-spec do_delete(trie(GZG, GZH), list(GZG)) -> gleam@option:option(trie(GZG, GZH)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec delete(trie(GYZ, GZA), list(GYZ)) -> trie(GYZ, GZA).
delete(Trie, Path) ->
...
-spec fold(trie(GZO, GZP), GZS, fun((GZS, list(GZO), GZP) -> GZS)) -> GZS.
...
-spec insert(trie(HAM, HAN), list(HAM), HAN) -> trie(HAM, HAN).
...
-spec from_list(list({list(GZU), GZW})) -> trie(GZU, GZW).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec get(trie(HAA, HAB), list(HAA)) -> {ok, HAB} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(HAH, any()), list(HAH)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(HAX, HAY), fun((HAY) -> HBB)) -> trie(HAX, HBB).
map(Trie, Fun) ->
    {trie,
...
-spec paths(trie(HBI, any())) -> list(list(HBI)).
paths(Trie) ->
...
-spec singleton(list(HBO), HBQ) -> trie(HBO, HBQ).
...
-spec subtrie(trie(HBX, HBY), list(HBX)) -> {ok, trie(HBX, HBY)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
...
-spec to_list(trie(HCG, HCH)) -> list({list(HCG), HCH}).
to_list(Trie) ->
...
-spec do_update(
    trie(HCV, HCW),
    list(HCV),
    fun((gleam@option:option(HCW)) -> gleam@option:option(HCW))
) -> gleam@option:option(trie(HCV, HCW)).
...
-spec update(
    trie(HCM, HCN),
    list(HCM),
    fun((gleam@option:option(HCN)) -> gleam@option:option(HCN))
) -> trie(HCM, HCN).
...
-spec values(trie(any(), HDG)) -> list(HDG).
values(Trie) ->
