...
-spec be_ok({ok, QGA} | {error, any()}) -> QGA.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, QGF}) -> QGF.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(QGI)) -> QGI.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
