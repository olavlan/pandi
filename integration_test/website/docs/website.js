...
function update2(model, message2) {
  if (message2 instanceof UserNavigatedTo) {
    ...
    return [new Model(model.posts, route), none()];
  } else {
    let $ = message2[0];
    if ($ instanceof Ok) {
      let body = $[0];
...
var FILEPATH2 = "src/website.gleam";
function main2() {
  let app = application(init2, update2, view2);
  let $ = start4(app, "#app", undefined);
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH2, "website", 7, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 126, end: 175, pattern_start: 137, pattern_end: 142 });
  }
  return;
}
